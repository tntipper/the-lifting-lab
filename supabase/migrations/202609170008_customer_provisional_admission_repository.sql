...
BEGIN
  ...
  IF NOT (SELECT rolsuper FROM pg_roles WHERE rolname=current_user) THEN
    CREATE ROLE tll_provisional_role_setup NOLOGIN NOINHERIT NOSUPERUSER NOCREATEDB CREATEROLE NOREPLICATION NOBYPASSRLS;
    EXECUTE format('GRANT tll_provisional_role_setup TO %I WITH INHERIT FALSE, SET TRUE',migration_role);
    SET LOCAL ROLE tll_provisional_role_setup;
  END IF;
...
BEGIN
  IF EXISTS(SELECT FROM pg_roles WHERE rolname='tll_provisional_role_setup') THEN
    SET LOCAL ROLE tll_provisional_role_setup;
    EXECUTE format('REVOKE tll_provisional_owner FROM %I',migration_role);
    EXECUTE format('SET LOCAL ROLE %I',migration_role);
    DROP ROLE tll_provisional_role_setup;
  ELSE
    EXECUTE format('REVOKE tll_provisional_owner FROM %I',migration_role);
  END IF;
  IF EXISTS(SELECT FROM pg_auth_members WHERE
    (roleid IN (SELECT oid FROM pg_roles WHERE rolname IN ('tll_provisional_owner','tll_provisional_executor'))
     OR member IN (SELECT oid FROM pg_roles WHERE rolname IN ('tll_provisional_owner','tll_provisional_executor')))
    AND NOT (roleid='tll_provisional_executor'::regrole AND member=migration_role::regrole
      AND admin_option AND NOT inherit_option AND NOT set_option))
    OR (SELECT count(*) FROM pg_auth_members WHERE roleid='tll_provisional_executor'::regrole AND member=migration_role::regrole
