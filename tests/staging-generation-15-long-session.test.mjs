...
test('generation 15 native gates are disarmed after bridge own_probe recovery', () => {
  assert.equal(NATIVE_GENERATION_15_TRANSPORT_ENABLED, false)
  assert.equal(NATIVE_GENERATION_15_DATABASE_TRANSPORT_ENABLED, false)
})
...
test('disabled launcher CLI returns NATIVE_TRANSPORT_DISABLED without long-session env', () => {
  ...
  assert.equal(payload.generation, 15)
})
