...
test('generation 9 native database transport and keychain access are disarmed after recovery',()=>{
  const helper=readFileSync('scripts/staging-generation-9-keychain.py','utf8')
  assert.equal(normalizeSupabaseToken(token),token);assert.equal(NATIVE_GENERATION_9_DATABASE_TRANSPORT_ENABLED,false);assert.equal(KEYCHAIN_HELPER_TIMEOUT_MS,45_000)
