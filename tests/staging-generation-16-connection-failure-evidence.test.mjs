...
} from '../scripts/staging-generation-16-connection-failure-evidence.mjs'
...
test('extractLauncherTerminalFromStdout recovers enriched connectionFailure from launcher line', () => {
  const stdout = [
    '{"event":"run-live-once-start","generation":16}',
...
test('generation 16 live launcher and run-live-once wire secret-free evidence persistence', () => {
  const launcher = readFileSync('scripts/staging-generation-16-live-launcher.mjs', 'utf8')
  const runOnce = readFileSync('scripts/staging-generation-16-run-live-once.mjs', 'utf8')
  ...
  assert.match(launcher, /verifyGeneration16ZeroSessionsAfterPoolerDrain/)
