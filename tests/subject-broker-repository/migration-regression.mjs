...
admin(`BEGIN;
 ...
 ALTER DEFAULT PRIVILEGES GRANT USAGE ON SCHEMAS TO tll_broker_default_probe;
 ALTER DEFAULT PRIVILEGES GRANT SELECT,INSERT,UPDATE,DELETE,TRUNCATE,REFERENCES,TRIGGER,MAINTAIN ON TABLES TO tll_broker_default_probe;
 ...
 DO $$DECLARE result jsonb; BEGIN
   ...
   result:=tll_broker_private.operator_set_enabled(true,'synthetic_acceptance');
   IF result->'enabled'<>'true'::jsonb THEN RAISE EXCEPTION 'Operator enable unavailable'; END IF;
   result:=tll_broker_private.operator_set_enabled(false,'synthetic_complete');
   IF result->'enabled'<>'false'::jsonb THEN RAISE EXCEPTION 'Operator disable unavailable'; END IF;
   BEGIN PERFORM * FROM tll_broker_private.flows; RAISE EXCEPTION 'Operator reads ciphertext'; EXCEPTION WHEN insufficient_privilege THEN NULL; END;
   BEGIN UPDATE tll_broker_private.control SET enabled=true; RAISE EXCEPTION 'Operator directly writes control'; EXCEPTION WHEN insufficient_privilege THEN NULL; END;
   BEGIN CREATE TABLE tll_broker_private.unexpected_operator_table(id int); RAISE EXCEPTION 'Operator can create'; EXCEPTION WHEN insufficient_privilege THEN NULL; END;
   BEGIN SET ROLE tll_broker_executor; RAISE EXCEPTION 'Operator can SET executor role'; EXCEPTION WHEN insufficient_privilege THEN NULL; END;
   BEGIN PERFORM tll_broker_private.repository('cancel','{}'::jsonb); RAISE EXCEPTION 'Operator can call executor'; EXCEPTION WHEN insufficient_privilege THEN NULL; END;
   BEGIN PERFORM tll_broker_private.operator_set_enabled(NULL,'synthetic'); RAISE EXCEPTION 'Null enable accepted'; EXCEPTION WHEN invalid_parameter_value THEN NULL; END;
   BEGIN PERFORM tll_broker_private.operator_set_enabled(true,'Invalid reason with spaces'); RAISE EXCEPTION 'Invalid reason accepted'; EXCEPTION WHEN invalid_parameter_value THEN NULL; END;
 END$$;
 ...
 CREATE ROLE tll_broker_runtime_probe NOLOGIN NOINHERIT NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS;
 GRANT tll_broker_executor TO tll_broker_runtime_probe WITH ADMIN FALSE, INHERIT TRUE, SET FALSE;
 DO $$BEGIN IF NOT pg_has_role('tll_broker_runtime_probe','tll_broker_executor','USAGE') THEN RAISE EXCEPTION 'Runtime delegation failed'; END IF; END$$;
 REVOKE tll_broker_executor FROM tll_broker_runtime_probe;
 DO $$BEGIN IF pg_has_role('tll_broker_runtime_probe','tll_broker_executor','MEMBER') THEN RAISE EXCEPTION 'Runtime revocation failed'; END IF; END$$;
 RESET SESSION AUTHORIZATION;
 CREATE ROLE tll_broker_operator_probe NOLOGIN;
 GRANT tll_broker_migrator TO tll_broker_operator_probe WITH INHERIT TRUE, SET TRUE;
 SET SESSION AUTHORIZATION tll_broker_operator_probe;
 SET ROLE tll_broker_migrator;
 DO $$BEGIN
   BEGIN PERFORM tll_broker_private.operator_status(); RAISE EXCEPTION 'SET ROLE impersonated operator status'; EXCEPTION WHEN insufficient_privilege THEN NULL; END;
   BEGIN PERFORM tll_broker_private.operator_set_enabled(true,'synthetic'); RAISE EXCEPTION 'SET ROLE impersonated operator control'; EXCEPTION WHEN insufficient_privilege THEN NULL; END;
 END$$;
 RESET SESSION AUTHORIZATION;
 ${['anon','authenticated','service_role','tll_broker_executor'].map(role=>`SET SESSION AUTHORIZATION ${role};
 DO $$BEGIN
   BEGIN PERFORM tll_broker_private.operator_status(); RAISE EXCEPTION 'Non-operator reads status'; EXCEPTION WHEN insufficient_privilege THEN NULL; END;
   BEGIN PERFORM tll_broker_private.operator_set_enabled(true,'synthetic'); RAISE EXCEPTION 'Non-operator controls use'; EXCEPTION WHEN insufficient_privilege THEN NULL; END;
