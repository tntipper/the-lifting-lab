...
test('classifyZeroSessionsFailure maps secret-free message classes', () => {
  assert.equal(classifyZeroSessionsFailure(Error('Generation 17 runtime sessions remain')), 'runtime_sessions_remain')
  assert.equal(classifyZeroSessionsFailure(Error('Generation 17 control enabled during zero-session proof')), 'control_enabled')
  assert.equal(classifyZeroSessionsFailure(Error('Generation-17 zero-session receipt mismatch')), 'receipt_mismatch')
  assert.equal(classifyZeroSessionsFailure(Error('Generation-17 database transport unavailable')), 'unavailable')
})

test('classifyZeroSessionsFailure walks cause chain for allow-listed SQL phrases', () => {
  const wrapped = Error('Generation-17 database transport unavailable')
  wrapped.cause = Error('ERROR: Generation 17 runtime sessions remain')
...
test('extractAllowListedSqlExceptionMessage promotes only allow-listed Management API phrases', () => {
  assert.equal(
    extractAllowListedSqlExceptionMessage(JSON.stringify({ message: 'ERROR: Generation 17 runtime sessions remain\nDETAIL: x' })),
    'Generation 17 runtime sessions remain',
  )
  assert.equal(
    extractAllowListedSqlExceptionMessage(JSON.stringify({ error: 'Generation 17 control enabled during zero-session proof' })),
    'Generation 17 control enabled during zero-session proof',
  )
  assert.equal(extractAllowListedSqlExceptionMessage(JSON.stringify({ message: 'permission denied for table secrets' })), undefined)
  assert.equal(extractAllowListedSqlExceptionMessage('not-json but runtime sessions remain here'), 'Generation 17 runtime sessions remain')
})
...
test('postManagementQuery preserves allow-listed SQL RAISE text from non-201 JSON bodies', async () => {
  const request = (_options, onResponse) => {
    ...
    queueMicrotask(() => {
      onResponse(response)
      listeners.get('data')?.(Buffer.from(JSON.stringify({ message: 'ERROR: Generation 17 runtime sessions remain' })))
      listeners.get('end')?.()
    })
    return {
      on() { return this },
...
test('happy path still requires ZERO_SESSIONS receipt after initial pooler drain', async () => {
  const pauses = []
  let posts = 0
  const value = await verifyGeneration17ZeroSessionsAfterPoolerDrain({
    token,
    convergenceMs: 7,
    pause: async ms => { pauses.push(ms) },
    post: async () => {
      posts += 1
      return [{ tll_generation_17_zero_sessions: receipt }]
    },
  })
...
test('transient runtime_sessions_remain retries with bounded pooler drain then passes', async () => {
  ...
  const value = await verifyGeneration17ZeroSessionsAfterPoolerDrain({
    ...
    post: async () => {
      posts += 1
      if (posts < 3) throw new Error('Generation 17 runtime sessions remain')
      return [{ tll_generation_17_zero_sessions: receipt }]
    },
  })
  assert.deepEqual(value, receipt)
...
test('exhausted runtime_sessions_remain still surfaces failureStep zero_sessions', async () => {
  ...
  await assert.rejects(
    () => verifyGeneration17ZeroSessionsAfterPoolerDrain({
      ...
      convergenceMs: 5,
      pause: async ms => { pauses.push(ms) },
      post: async () => { throw new Error('Generation 17 runtime sessions remain') },
    }),
    error => {
...
test('permanent control_enabled fails immediately with failureStep zero_sessions', async () => {
  ...
  await assert.rejects(
    () => verifyGeneration17ZeroSessionsAfterPoolerDrain({
      ...
      post: async () => {
        posts += 1
        throw new Error('Generation 17 control enabled during zero-session proof')
      },
    }),
    error => {
      assert.equal(error.failureStep, 'zero_sessions')
...
test('direct zero-session proof still demands exact ZERO_SESSIONS receipt', async () => {
  await assert.rejects(
    () => verifyGeneration17ZeroSessions({
      token,
      post: async () => [{ tll_generation_17_zero_sessions: { ...receipt, status: 'NOT_ZERO' } }],
    }),
    /receipt mismatch/,
  )
})
