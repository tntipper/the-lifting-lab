...
import { buildGeneration7CredentialSql, createGeneration7DispatchJournal, GENERATION, IDENTITIES, INERT_VALID_UNTIL, PACKAGE_ID, PREDECESSOR, PROJECT_REF, WINDOW_ID } from '../scripts/staging-generation-7-credentials.mjs'
...
test('generation 7 SQL requires exact retired generation 6 and remains one disabled staging transaction',()=>{
  const values=verifiers(),sql=buildGeneration7CredentialSql({expiresAt:EXPIRES,verifiers:values,nowMs:NOW})
  assert.equal(GENERATION,7);assert.equal(PREDECESSOR.generation,6);assert.ok(sql.startsWith('BEGIN;\n'));assert.equal((sql.match(/^COMMIT;$/gm)??[]).length,1)
...
test('generation 7 expiry and verifier contract fails closed',()=>{
  assert.throws(()=>buildGeneration7CredentialSql({expiresAt:'2026-09-20T21:00:01.000Z',verifiers:verifiers(),nowMs:NOW}),/unavailable/)
  const missing=verifiers();delete missing.bridge;assert.throws(()=>buildGeneration7CredentialSql({expiresAt:EXPIRES,verifiers:missing,nowMs:NOW}),/unavailable/)
  const duplicate=verifiers();duplicate.bridge=duplicate.customer;assert.throws(()=>buildGeneration7CredentialSql({expiresAt:EXPIRES,verifiers:duplicate,nowMs:NOW}),/unavailable/)
})

test('generation 7 journal is separate, exclusive, mode 0600 and terminal',()=>{
  const directory=mkdtempSync(join(tmpdir(),'tll-gen7-')),path=join(directory,'journal.json'),journal=createGeneration7DispatchJournal({path,makeRunId:()=> 'reviewed-generation-7-run'})
