...
export const CONTAINER='tll-stage0-postgres',DATABASE='tll_customer_repository'
...
export function admin(sql){
 ...
 catch{throw new Error('Synthetic customer repository SQL failed (raw output suppressed)')}
}
...
export function assertFixture(){
 ...
 if(admin("SELECT current_database()||':'||coalesce(shobj_description(oid,'pg_database'),'') FROM pg_database WHERE datname=current_database();")!==DATABASE+':tll-customer-repository-synthetic-v1')throw new Error('Missing exact synthetic database marker')
}
...
export function localPool({role='tll_customer_executor',fault}={}){
 if(!['tll_customer_executor','postgres','anon','authenticated','service_role'].includes(role))throw new Error('Invalid synthetic role')
 return {async connect(){const c=new LocalClient(role);await c.ready;const query=c.query.bind(c);let op;return {async query(sql,values){if(values)op=values[0];const r=await query(sql,values);if(fault?.({sql,op,rows:r.rows}))throw new Error('Synthetic lost acknowledgement');return r},release:c.release.bind(c)}}}
}
