...
} from '../scripts/staging-generation-18-connection-failure-evidence.mjs'
...
test('secretFreeLauncherTerminal includes enriched connectionFailure without secrets', () => {
  ...
  assert.equal(terminal.generation, 18)
...
test('generation 18 live launcher and run-live-once wire secret-free evidence persistence', () => {
  const launcher = readFileSync('scripts/staging-generation-18-live-launcher.mjs', 'utf8')
  const runOnce = readFileSync('scripts/staging-generation-18-run-live-once.mjs', 'utf8')
  ...
  assert.match(launcher, /verifyGeneration18ZeroSessionsAfterPoolerDrain/)
