...
} from '../scripts/staging-generation-21-connection-failure-evidence.mjs'
...
test('secretFreeLauncherTerminal includes enriched connectionFailure without secrets', () => {
  const terminal = secretFreeLauncherTerminal({
    ...
    generation: 21,
    windowId: 'a5511645-77af-4fc9-9e4c-f5c8a474d5fa',
  ...
  assert.equal(terminal.generation, 21)
...
test('generation 21 live launcher and run-live-once wire secret-free evidence persistence', () => {
  const launcher = readFileSync('scripts/staging-generation-21-live-launcher.mjs', 'utf8')
  const runOnce = readFileSync('scripts/staging-generation-21-run-live-once.mjs', 'utf8')
  ...
  assert.match(launcher, /verifyGeneration21ZeroSessionsAfterPoolerDrain/)
