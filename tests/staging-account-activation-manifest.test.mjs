...
test('staging account activation manifest pins reviewed sources and contains no secret values', () => {
  ...
  assert.equal(manifest.stageSafety.generation19Armed, true)
...
test('generation 19 successor is pinned with Gen 17 role predecessor, Gen 18 attempt superseded, prevention artefacts, and native gates armed for one window', () => {
  execFileSync(process.execPath, ['scripts/staging-account-activation-manifest.mjs', '--check'], { stdio: 'pipe' })
  const manifest = JSON.parse(readFileSync('config/staging-account-activation-manifest.json', 'utf8'))
  assert.equal(manifest.stageSafety.generation19Armed, true)
  ...
  assert.equal(manifest.generation19Successor.nativeTransportEnabled, true)
  assert.equal(manifest.generation19Successor.status, 'ONE_STAGING_WINDOW_AUTHORIZED')
