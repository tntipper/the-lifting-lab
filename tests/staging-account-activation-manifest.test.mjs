...
test('staging account activation manifest pins reviewed sources and contains no secret values', () => {
  ...
  assert.equal(manifest.generation8Successor.nativeTransportEnabled, true)
  assert.equal(manifest.generation8Successor.status, 'ONE_STAGING_WINDOW_AUTHORIZED')
  ...
  for (const pin of [...manifest.migrations,...manifest.edge.sources,...manifest.runtime.sources,...manifest.preflight.sources,...manifest.disabledMigrationInstall.sources,...manifest.recovery.sources,...manifest.generation6Credentials.sources,...manifest.generation7Successor.sources,...manifest.generation8Successor.sources]) assert.match(pin.sha256,/^[a-f0-9]{64}$/)
