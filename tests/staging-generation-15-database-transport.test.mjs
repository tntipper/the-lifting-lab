...
test('generation 15 native database transport and keychain access are disarmed after bridge own_probe recovery',()=>{
  const helper=readFileSync('scripts/staging-generation-15-keychain.py','utf8')
  assert.equal(normalizeSupabaseToken(token),token);assert.equal(NATIVE_GENERATION_15_DATABASE_TRANSPORT_ENABLED,false);assert.equal(KEYCHAIN_HELPER_TIMEOUT_MS,45_000)
