...
import {
  ...
  KEEPALIVE_PATH_ENV,
  KEEPALIVE_SCHEMA,
} from '../scripts/staging-generation-21-long-session-contract.mjs'
...
test('generation 21 native gates are disarmed while disabled', () => {
...
test('assertLongSessionContract accepts valid keepalive held by parent', () => {
  ...
  const result = assertLongSessionContract({
    ...
    ppid: 42,
    readParentComm: () => 'node',
    readParentCmdline: () => 'node scripts/staging-generation-21-run-live-once.mjs',
  })
...
test('supported Gen21 operator entry requires explicit secret-free retirement evidence before creating a keepalive', async () => {
...
test('journal-watch supports --follow --interval without importing launcher', () => {
  const source = readFileSync('scripts/staging-generation-21-journal-watch.mjs', 'utf8')
  assert.match(source, /--follow/)
  assert.match(source, /--interval/)
  assert.doesNotMatch(source, /runNativeGeneration21CredentialWindow|readSupabaseTokenFromKeychain/)
})

test('live launcher wires long-session contract before armed journal claim', () => {
  const source = readFileSync('scripts/staging-generation-21-live-launcher.mjs', 'utf8')
  assert.match(source, /assertLongSessionContract/)
  assert.match(source, /staging-generation-21-long-session-contract\.mjs/)
  ...
  assert.match(source, /assertGeneration21PreArmReady\(\{ retirementEvidence: evidence, requireArmedGatesFalse: false \}\)/)
