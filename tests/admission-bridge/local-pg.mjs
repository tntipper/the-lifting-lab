...
export const CONTAINER='tll-stage0-postgres',DATABASE='tll_admission_bridge'
...
export function admin(sql){
 ...
 catch{throw new Error('Synthetic admission bridge SQL failed (raw output suppressed)')}
}
...
export function assertFixture(){
 ...
 if(admin("SELECT current_database()||':'||coalesce(shobj_description(oid,'pg_database'),'') FROM pg_database WHERE datname=current_database();")!==DATABASE+':tll-admission-bridge-synthetic-v1')throw new Error('Missing exact synthetic database marker')
}
...
export function localPool({role='tll_ab_bridge_executor',fault}={}){
 if(!['tll_ab_bridge_executor','tll_ab_broker_executor','tll_ab_provisional_executor','tll_admission_bridge_migrator','postgres','anon','authenticated','service_role'].includes(role))throw new Error('Invalid synthetic role')
