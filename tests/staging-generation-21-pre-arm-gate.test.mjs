...
test('Generation 21 identity, exact Gen19 predecessor, and disabled native gates are fixed', () => {
...
test('pre-arm gate permanently refuses the consumed Generation 21 window', async () => {
  assert.equal(GENERATION_21_CONSUMED, true)
  await assert.rejects(() => assertGeneration21PreArmReady(), /generation21_consumed_no_replay/)
  await assert.rejects(() => assertGeneration21PreArmReady({ retirementEvidence: RETIREMENT_EVIDENCE }), /generation21_consumed_no_replay/)
