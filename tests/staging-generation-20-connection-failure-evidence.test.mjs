...
} from '../scripts/staging-generation-20-connection-failure-evidence.mjs'
...
test('secretFreeLauncherTerminal includes enriched connectionFailure without secrets', () => {
  const terminal = secretFreeLauncherTerminal({
    ...
    generation: 20,
    windowId: 'a009f2b4-86df-4701-a8bc-1112597e3c42',
  ...
  assert.equal(terminal.generation, 20)
...
test('extractLauncherTerminalFromStdout recovers enriched connectionFailure from launcher line', () => {
  const stdout = [
    '{"event":"run-live-once-start","generation":20}',
...
test('generation 20 live launcher and run-live-once wire secret-free evidence persistence', () => {
  const launcher = readFileSync('scripts/staging-generation-20-live-launcher.mjs', 'utf8')
  const runOnce = readFileSync('scripts/staging-generation-20-run-live-once.mjs', 'utf8')
  ...
  assert.match(launcher, /verifyGeneration20ZeroSessionsAfterPoolerDrain/)
