...
test('generation 18 native gates remain disarmed (Phase 1 disabled successor)', () => {
  assert.equal(NATIVE_GENERATION_18_TRANSPORT_ENABLED, false)
  assert.equal(NATIVE_GENERATION_18_DATABASE_TRANSPORT_ENABLED, false)
})
...
test('disabled launcher CLI returns NATIVE_TRANSPORT_DISABLED without long-session env', () => {
  ...
  assert.equal(payload.generation, 18)
})
