...
const before=admin("SELECT json_build_object('control',(SELECT row_to_json(c) FROM tll_provisional_private.control c),'attempts',(SELECT coalesce(json_agg(x),'[]') FROM tll_provisional_private.operations x),'connections',(SELECT coalesce(json_agg(x),'[]') FROM tll_provisional_private.intents x));")
...
admin(`BEGIN;
 ...
 ROLLBACK;`)
assert.equal(admin("SELECT json_build_object('control',(SELECT row_to_json(c) FROM tll_provisional_private.control c),'attempts',(SELECT coalesce(json_agg(x),'[]') FROM tll_provisional_private.operations x),'connections',(SELECT coalesce(json_agg(x),'[]') FROM tll_provisional_private.intents x));"),before)
