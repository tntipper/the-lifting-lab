...
export const CONTAINER='tll-stage0-postgres',DATABASE='tll_provisional_repository'
...
export function admin(sql){
 ...
 catch{throw new Error('Synthetic provisional admission repository SQL failed (raw output suppressed)')}
}
...
export function assertFixture(){
 ...
 if(admin("SELECT current_database()||':'||coalesce(shobj_description(oid,'pg_database'),'') FROM pg_database WHERE datname=current_database();")!==DATABASE+':tll-provisional-admission-repository-synthetic-v1')throw new Error('Missing exact synthetic database marker')
}
...
export function localPool({role='tll_provisional_executor',fault}={}){
 if(!['tll_provisional_executor','postgres','anon','authenticated','service_role'].includes(role))throw new Error('Invalid synthetic role')
