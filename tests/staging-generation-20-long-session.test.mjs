...
import {
  ...
  KEEPALIVE_PATH_ENV,
  KEEPALIVE_SCHEMA,
} from '../scripts/staging-generation-20-long-session-contract.mjs'
...
import { assertSupportedOperatorEntry, runLiveOnceMain } from '../scripts/staging-generation-20-run-live-once.mjs'

test('generation 20 native gates match the reviewed one-window arming diff', () => {
...
test('armed launcher CLI rejects a missing long-session contract before native work', () => {
  ...
  assert.match(result.stderr, /long-session/i)
})
...
test('assertLongSessionContract accepts valid keepalive held by parent', () => {
  ...
  const result = assertLongSessionContract({
    ...
    ppid: 42,
    readParentComm: () => 'node',
    readParentCmdline: () => 'node scripts/staging-generation-20-run-live-once.mjs',
  })
...
test('supported Gen20 operator entry requires explicit secret-free retirement evidence before creating a keepalive', async () => {
  await assert.rejects(
    () => runLiveOnceMain([], { ppid: 42, env: {} }),
    error => error?.code === 'RUN_LIVE_ONCE_REJECTED' && /retirement-evidence/.test(error.message),
  )
})

test('journal-watch supports --follow --interval without importing launcher', () => {
  const source = readFileSync('scripts/staging-generation-20-journal-watch.mjs', 'utf8')
  assert.match(source, /--follow/)
  assert.match(source, /--interval/)
  assert.doesNotMatch(source, /runNativeGeneration20CredentialWindow|readSupabaseTokenFromKeychain/)
})

test('live launcher wires long-session contract before armed journal claim', () => {
  const source = readFileSync('scripts/staging-generation-20-live-launcher.mjs', 'utf8')
  assert.match(source, /assertLongSessionContract/)
  assert.match(source, /staging-generation-20-long-session-contract\.mjs/)
  assert.match(source, /TLL_LIVE_LONG_SESSION/)
  assert.match(source, /gatesArmed\(\)/)
  assert.match(source, /RETIREMENT_EVIDENCE_PATH_ENV/)
  assert.match(source, /loadRetirementEvidenceFromPath/)
  assert.match(source, /assertGeneration20PreArmReady\(\{ retirementEvidence: evidence, requireArmedGatesFalse: false \}\)/)
