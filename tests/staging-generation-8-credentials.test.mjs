...
test('generation 8 SQL requires exact retired generation 6 and remains one disabled staging transaction',()=>{
  const values=verifiers(),sql=buildGeneration8CredentialSql({expiresAt:EXPIRES,verifiers:values,nowMs:NOW})
  assert.equal(GENERATION,8);assert.equal(PREDECESSOR.generation,6);assert.ok(sql.startsWith('BEGIN;\n'));assert.equal((sql.match(/^COMMIT;$/gm)??[]).length,1)
...
test('generation 8 expiry and verifier contract fails closed',()=>{
  assert.throws(()=>buildGeneration8CredentialSql({expiresAt:'2026-09-20T21:00:01.000Z',verifiers:verifiers(),nowMs:NOW}),/unavailable/)
  const missing=verifiers();delete missing.bridge;assert.throws(()=>buildGeneration8CredentialSql({expiresAt:EXPIRES,verifiers:missing,nowMs:NOW}),/unavailable/)
  const duplicate=verifiers();duplicate.bridge=duplicate.customer;assert.throws(()=>buildGeneration8CredentialSql({expiresAt:EXPIRES,verifiers:duplicate,nowMs:NOW}),/unavailable/)
})

test('generation 8 journal is separate, exclusive, mode 0600 and terminal',()=>{
  const directory=mkdtempSync(join(tmpdir(),'tll-gen8-')),path=join(directory,'journal.json'),journal=createGeneration8DispatchJournal({path,makeRunId:()=> 'reviewed-generation-8-run'})
