...
test('generation 13 native database transport and keychain access are armed for one reviewed window',()=>{
  const helper=readFileSync('scripts/staging-generation-13-keychain.py','utf8')
  assert.equal(normalizeSupabaseToken(token),token);assert.equal(NATIVE_GENERATION_13_DATABASE_TRANSPORT_ENABLED,true);assert.equal(KEYCHAIN_HELPER_TIMEOUT_MS,45_000)
