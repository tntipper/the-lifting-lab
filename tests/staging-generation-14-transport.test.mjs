...
import { PACKAGE_ID, PROJECT_REF, WINDOW_ID } from '../scripts/staging-generation-14-credentials.mjs'
...
import { executeGeneration14CredentialWindow, GENERATION_14_SOURCE_FINGERPRINT, NATIVE_GENERATION_14_TRANSPORT_ENABLED } from '../scripts/staging-generation-14-transport.mjs'
...
function fixture(fail){const events=[],ports={
  ...
  async dispatchDatabase(){events.push('dispatch');return[{tll_generation_14_credential_receipt:{status:'PASS',packageId:PACKAGE_ID,projectRef:PROJECT_REF,generation:14,windowId:WINDOW_ID,expiresAt:EXPIRES,controlsEnabled:false,runtimeCount:5}}]},
  ...
  return{events,ports}}
const journal=events=>({recordIntent(value){events.push('intent');return{...value,state:'INTENT_RECORDED',runId:'generation-14-test'}},transition(_intent,state){events.push(state);return{state}}})
...
test('generation 14 is armed for one reviewed window with a fixed nonsecret fingerprint',()=>{
  assert.equal(NATIVE_GENERATION_14_TRANSPORT_ENABLED,true);assert.match(GENERATION_14_SOURCE_FINGERPRINT,/^[a-f0-9]{64}$/)
})

test('generation 14 consumed transport contains no native launcher or credential-bearing imports',()=>{
  assert.equal(NATIVE_GENERATION_14_TRANSPORT_ENABLED,true)
  const source=readFileSync('scripts/staging-generation-14-transport.mjs','utf8')
  assert.doesNotMatch(source,/runNativeGeneration14CredentialWindow/)
...
test('generation 14 success stages providers, journals once and verifies before receipt',async()=>{
  const f=fixture(),result=await executeGeneration14CredentialWindow({ports:f.ports,journal:journal(f.events),now:()=>NOW,randomBytes,randomUUID})
  assert.equal(result.status,'CREDENTIALS_VERIFIED_CONTROLS_DISABLED');assert.equal(result.generation,14);assert.deepEqual(f.events,['preflight','intent','stageVercel','stageSupabase','readback','dispatch','verify','RECEIPT_VALIDATED'])
})

test('a consumed journal is rejected before material, provider, database, recovery, or cleanup work',async()=>{
  const f=fixture(),result=await executeGeneration14CredentialWindow({ports:f.ports,journal:rejectedJournal(f.events),now:()=>NOW,
...
test('generation 14 connection failure recovers once, projects fixed diagnostic and forbids replay',async()=>{
  let byte=0;const f=fixture('verify'),result=await executeGeneration14CredentialWindow({ports:f.ports,journal:journal(f.events),now:()=>NOW,randomBytes:size=>Buffer.alloc(size,++byte),randomUUID:(()=>{let n=0;return()=>`10000000-0000-4000-8000-${String(++n).padStart(12,'0')}`})()})
...
test('generation 14 embeds SCRAM derived from projected passwords into dispatch SQL',async()=>{
  ...
  const f=fixture()
  f.ports.dispatchDatabase=async sql=>{f.events.push('dispatch');capturedSql=sql;return[{tll_generation_14_credential_receipt:{status:'PASS',packageId:PACKAGE_ID,projectRef:PROJECT_REF,generation:14,windowId:WINDOW_ID,expiresAt:EXPIRES,controlsEnabled:false,runtimeCount:5}}]}
  ...
  await executeGeneration14CredentialWindow({ports:f.ports,journal:journal(f.events),now:()=>NOW,randomBytes,randomUUID})
