...
export async function verifyStackAssessment(page, origin, width, resultDir) {
  ...
  await page.getByText(/Historical average: 80\/100 across 1 of 5 products/).waitFor()
  const mainText = await page.locator('main').innerText()
  assert.match(mainText, /1 under review and 3 unassessed or unavailable excluded/)
  ...
  assert.match(email.searchParams.get('body'), /Historical average: 80\/100 across 1 of 5/)
  assert.match(email.searchParams.get('body'), /Under review; historical value 99\/100/)
  ...
  assert.match(x.searchParams.get('text'), /1 of 5 products/)
  ...
  await dialog.getByText('99/100').waitFor()
  const text = await dialog.innerText()
  assert.match(text, /Under review/); assert.match(text, /Not assessed/)
  assert.doesNotMatch(text, /100\/100|Excellent|Good dosing/)
