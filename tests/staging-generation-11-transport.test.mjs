...
test('generation 11 is armed for one reviewed window with a fixed nonsecret fingerprint',()=>{
  assert.equal(NATIVE_GENERATION_11_TRANSPORT_ENABLED,true);assert.match(GENERATION_11_SOURCE_FINGERPRINT,/^[a-f0-9]{64}$/)
})

test('generation 11 consumed transport contains no native launcher or credential-bearing imports',()=>{
  assert.equal(NATIVE_GENERATION_11_TRANSPORT_ENABLED,true)
