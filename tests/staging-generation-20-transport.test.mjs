...
import { ACTIVE_WINDOW_EXPIRES_AT, PROJECT_REF, WINDOW_ID } from '../scripts/staging-generation-20-credentials.mjs'
import { executeGeneration20CredentialWindow, GENERATION_20_SOURCE_FINGERPRINT, NATIVE_GENERATION_20_TRANSPORT_ENABLED } from '../scripts/staging-generation-20-transport.mjs'

test('Gen20 transport uses the shared phase-1 expiry source and has no dynamic expiry calculation', () => {
...
test('phase-1 expiry sentinel blocks before preflight, journal, material, provider, or database actions', async () => {
  const events = []
  ...
  const result = await executeGeneration20CredentialWindow({ ports, journal })
  assert.deepEqual(result, { status:'WINDOW_EXPIRY_UNSET', target:PROJECT_REF, generation:20, windowId:WINDOW_ID,
