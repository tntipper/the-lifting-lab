...
test('generation 13 native gates are disarmed after connection recovery', () => {
  assert.equal(NATIVE_GENERATION_13_TRANSPORT_ENABLED, false)
  assert.equal(NATIVE_GENERATION_13_DATABASE_TRANSPORT_ENABLED, false)
})
...
test('disabled launcher CLI returns NATIVE_TRANSPORT_DISABLED without long-session env', () => {
  ...
  assert.equal(payload.generation, 13)
})
