...
import {
  ...
  KEEPALIVE_PATH_ENV,
  KEEPALIVE_SCHEMA,
} from '../scripts/staging-generation-13-long-session-contract.mjs'
import { NATIVE_GENERATION_13_TRANSPORT_ENABLED } from '../scripts/staging-generation-13-transport.mjs'
import { NATIVE_GENERATION_13_DATABASE_TRANSPORT_ENABLED } from '../scripts/staging-generation-13-database-transport.mjs'
import { assertSupportedOperatorEntry } from '../scripts/staging-generation-13-run-live-once.mjs'

test('generation 13 native gates are armed for one reviewed window', () => {
  assert.equal(NATIVE_GENERATION_13_TRANSPORT_ENABLED, true)
  assert.equal(NATIVE_GENERATION_13_DATABASE_TRANSPORT_ENABLED, true)
})
...
test('assertLongSessionContract accepts valid keepalive held by parent', () => {
  ...
  const result = assertLongSessionContract({
    ...
    ppid: 42,
    readParentComm: () => 'node',
    readParentCmdline: () => 'node scripts/staging-generation-13-run-live-once.mjs',
  })
...
test('journal-watch supports --follow --interval without importing launcher', () => {
  const source = readFileSync('scripts/staging-generation-13-journal-watch.mjs', 'utf8')
  assert.match(source, /--follow/)
  assert.match(source, /--interval/)
  assert.doesNotMatch(source, /runNativeGeneration13CredentialWindow|readSupabaseTokenFromKeychain/)
})
...
test('live launcher wires long-session contract before armed journal claim', () => {
  ...
  assert.match(source, /staging-generation-13-long-session-contract\.mjs/)
