...
} from '../scripts/staging-generation-19-connection-failure-evidence.mjs'
...
test('secretFreeLauncherTerminal includes enriched connectionFailure without secrets', () => {
  ...
  assert.equal(terminal.generation, 19)
...
test('generation 19 live launcher and run-live-once wire secret-free evidence persistence', () => {
  const launcher = readFileSync('scripts/staging-generation-19-live-launcher.mjs', 'utf8')
  const runOnce = readFileSync('scripts/staging-generation-19-run-live-once.mjs', 'utf8')
  ...
  assert.match(launcher, /verifyGeneration19ZeroSessionsAfterPoolerDrain/)
