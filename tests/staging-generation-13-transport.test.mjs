...
function fixture(fail){const events=[],ports={
  ...
  async dispatchDatabase(){events.push('dispatch');return[{tll_generation_13_credential_receipt:{status:'PASS',packageId:PACKAGE_ID,projectRef:PROJECT_REF,generation:13,windowId:WINDOW_ID,expiresAt:EXPIRES,controlsEnabled:false,runtimeCount:5}}]},
  ...
  return{events,ports}}
const journal=events=>({recordIntent(value){events.push('intent');return{...value,state:'INTENT_RECORDED',runId:'generation-13-test'}},transition(_intent,state){events.push(state);return{state}}})
...
test('generation 13 is disarmed after interrupted recovery with a fixed nonsecret fingerprint',()=>{
  assert.equal(NATIVE_GENERATION_13_TRANSPORT_ENABLED,false);assert.match(GENERATION_13_SOURCE_FINGERPRINT,/^[a-f0-9]{64}$/)
})

test('generation 13 consumed transport contains no native launcher or credential-bearing imports',()=>{
  assert.equal(NATIVE_GENERATION_13_TRANSPORT_ENABLED,false)
  const source=readFileSync('scripts/staging-generation-13-transport.mjs','utf8')
  assert.doesNotMatch(source,/runNativeGeneration13CredentialWindow/)
...
test('generation 13 success stages providers, journals once and verifies before receipt',async()=>{
  const f=fixture(),result=await executeGeneration13CredentialWindow({ports:f.ports,journal:journal(f.events),now:()=>NOW,randomBytes,randomUUID})
  assert.equal(result.status,'CREDENTIALS_VERIFIED_CONTROLS_DISABLED');assert.equal(result.generation,13);assert.deepEqual(f.events,['preflight','intent','stageVercel','stageSupabase','readback','dispatch','verify','RECEIPT_VALIDATED'])
})

test('a consumed journal is rejected before material, provider, database, recovery, or cleanup work',async()=>{
  const f=fixture(),result=await executeGeneration13CredentialWindow({ports:f.ports,journal:rejectedJournal(f.events),now:()=>NOW,
...
test('generation 13 connection failure recovers once, projects fixed diagnostic and forbids replay',async()=>{
