...
test('generation 13 is armed for one reviewed window with a fixed nonsecret fingerprint',()=>{
  assert.equal(NATIVE_GENERATION_13_TRANSPORT_ENABLED,true);assert.match(GENERATION_13_SOURCE_FINGERPRINT,/^[a-f0-9]{64}$/)
})

test('generation 13 consumed transport contains no native launcher or credential-bearing imports',()=>{
  assert.equal(NATIVE_GENERATION_13_TRANSPORT_ENABLED,true)
