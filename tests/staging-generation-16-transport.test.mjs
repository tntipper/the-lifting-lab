...
import { PACKAGE_ID, PROJECT_REF, WINDOW_ID } from '../scripts/staging-generation-16-credentials.mjs'
...
import { executeGeneration16CredentialWindow, GENERATION_16_SOURCE_FINGERPRINT, NATIVE_GENERATION_16_TRANSPORT_ENABLED } from '../scripts/staging-generation-16-transport.mjs'
...
function fixture(fail){const events=[],ports={
  ...
  async dispatchDatabase(){events.push('dispatch');return[{tll_generation_16_credential_receipt:{status:'PASS',packageId:PACKAGE_ID,projectRef:PROJECT_REF,generation:16,windowId:WINDOW_ID,expiresAt:EXPIRES,controlsEnabled:false,runtimeCount:5}}]},
...
test('generation 16 is disarmed after zero-sessions unavailable recovery with a fixed nonsecret fingerprint',()=>{
...
test('generation 16 success stages providers, journals once and verifies before receipt',async()=>{
  const f=fixture(),result=await executeGeneration16CredentialWindow({ports:f.ports,journal:journal(f.events),now:()=>NOW,randomBytes,randomUUID})
  assert.equal(result.status,'CREDENTIALS_VERIFIED_CONTROLS_DISABLED');assert.equal(result.generation, 16);assert.deepEqual(f.events,['preflight','intent','stageVercel','stageSupabase','readback','dispatch','verify','RECEIPT_VALIDATED'])
})

test('a consumed journal is rejected before material, provider, database, recovery, or cleanup work',async()=>{
  const f=fixture(),result=await executeGeneration16CredentialWindow({ports:f.ports,journal:rejectedJournal(f.events),now:()=>NOW,
...
test('generation 16 connection failure recovers once, projects fixed diagnostic and forbids replay',async()=>{
  let byte=0;const f=fixture('verify'),result=await executeGeneration16CredentialWindow({ports:f.ports,journal:journal(f.events),now:()=>NOW,randomBytes:size=>Buffer.alloc(size,++byte),randomUUID:(()=>{let n=0;return()=>`10000000-0000-4000-8000-${String(++n).padStart(12,'0')}`})()})
...
test('generation 16 embeds SCRAM derived from projected passwords into dispatch SQL',async()=>{
  ...
  const f=fixture()
  f.ports.dispatchDatabase=async sql=>{f.events.push('dispatch');capturedSql=sql;return[{tll_generation_16_credential_receipt:{status:'PASS',packageId:PACKAGE_ID,projectRef:PROJECT_REF,generation:16,windowId:WINDOW_ID,expiresAt:EXPIRES,controlsEnabled:false,runtimeCount:5}}]}
  ...
  await executeGeneration16CredentialWindow({ports:f.ports,journal:journal(f.events),now:()=>NOW,randomBytes,randomUUID})
