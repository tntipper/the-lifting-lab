...
test('generation 12 native database transport and keychain access are disarmed after interrupted recovery',()=>{
  const helper=readFileSync('scripts/staging-generation-12-keychain.py','utf8')
  assert.equal(normalizeSupabaseToken(token),token);assert.equal(NATIVE_GENERATION_12_DATABASE_TRANSPORT_ENABLED,false);assert.equal(KEYCHAIN_HELPER_TIMEOUT_MS,45_000)
