...
import { FIXTURE_LABEL, startStackRelay } from './loopback-relay.mjs'
...
const password = 'tll-stack-synthetic-only-database'
const jwtSecret = 'tll-stack-synthetic-only-jwt-secret-000000'
const database = 'tll_stack_test'

export function namesFor(id) {
  if (!/^[a-f0-9]{12}$/.test(id)) throw new Error('Invalid synthetic run id')
  return { id, postgres: `tll-stack-ci-postgres-${id}`, postgrest: `tll-stack-ci-postgrest-${id}`, network: `tll-stack-ci-network-${id}` }
}
...
export async function runAcceptance({ execute = command, request = fetch, relayFactory = startStackRelay, pause = sleep, now = Date.now, signal, env = process.env, platform = process.platform, id = randomBytes(6).toString('hex'), log = console.log } = {}) {
  ...
  try {
    ...
    // This suite bootstraps only its synthetic DB and applies F09 once.
    await test('tests/stacks/database.test.mjs')
    await docker(['exec', '-i', names.postgres, 'psql', '-X', '-q', '-U', 'postgres', '-d', database, '-v', 'ON_ERROR_STOP=1'], { input: `create role tll_stack_authenticator noinherit login password '${password}'; grant anon,authenticated to tll_stack_authenticator;` })
    ...
    await ready(async () => {
      const response = await request(origin + '/rpc/get_active_stack', { headers: { Authorization: 'Bearer ' + probeToken }, signal: AbortSignal.timeout(3000) })
      return response.status === 200 && (await response.json()).userId === probeUser
    }, 'PostgREST schema cache', { pause, now, signal })
    await test('tests/stacks/http.test.mjs')
  ...
  log('PASS: stack SQL, HTTP acceptance and owned cleanup complete.')
}

async function main() {
  const mode = validateArguments(process.argv.slice(2), process.platform)
  if (mode !== 'run') {
    console.log(mode === '--help' ? 'Usage: node tests/stacks/run-local.mjs [--help|--plan]\nRuns only on Linux Docker. No custom services, credentials or targets are accepted.' : JSON.stringify({ images: IMAGES, database, network: 'generated, isolated, no external egress', postgresPublishedPorts: [], apiRelay: '127.0.0.1:<ephemeral> to verified task-container IP:3000', phases: ['SQL bootstrap + one-time migration', 'HTTP', 'owned resource cleanup'] }, null, 2))
    return
  }
  const controller = new AbortController()
  const cancel = () => controller.abort(new Error('Stack acceptance interrupted'))
