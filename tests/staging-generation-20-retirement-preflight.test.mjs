...
import { EXPIRES_AT, FIXED_QUERY, GENERATION, OUTPUT, PRODUCTION_PROJECT_REF, PROJECT_REF, QUERY_ID, WINDOW_ID, validateResult } from '../scripts/staging-generation-20-retirement-preflight.mjs'

test('Gen20 retirement preflight is fixed, read-only, and checked in', () => {
  execFileSync(process.execPath, ['scripts/staging-generation-20-retirement-preflight.mjs', '--check'])
  ...
  assert.ok(FIXED_QUERY.indexOf('AS tll_gen20_retirement_preflight;') < FIXED_QUERY.lastIndexOf('COMMIT;'), 'receipt must be read inside the read-only transaction')
...
test('validator accepts only the compact aggregate exact-active receipt', () => {
  ...
  assert.equal(validateResult([{ tll_gen20_retirement_preflight: receipt }]).status, 'PASS')
  assert.throws(() => validateResult([{ tll_gen20_retirement_preflight: { ...receipt, runtimeSessions: 1 } }]))
  assert.throws(() => validateResult([{ tll_gen20_retirement_preflight: { ...receipt, unexpected: true } }]))
  assert.throws(() => validateResult([{ tll_gen20_retirement_preflight: { ...receipt, workCounts: { ...workCounts, accountLogouts: -1 } } }]))
})
