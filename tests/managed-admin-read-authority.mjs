...
const fixtures = {
  customer: ['customer-repository', '202609180012_customer_shopify_proof_repository.sql'],
...
for (const name of chosen.length ? chosen : Object.keys(fixtures)) {
  ...
  const checkedGuards = name === 'bridge'
    ...
    : name === 'customer' ? canonical.match(/DO \$postflight\$[\s\S]*?END \$postflight\$;/)?.[0]
  ...
  const guards = name === 'bridge' || name === 'customer' ? checkedGuards
    : `GRANT ${owner} TO ${operator} WITH INHERIT TRUE,SET TRUE; ${checkedGuards}`
  admin(exclusive)
...
DROP OWNED BY ${owner},${executor}; DROP ROLE ${owner},${executor};`
    : name === 'customer' ? `DROP FUNCTION tll_customer_private.shopify_proof_repository(text,jsonb);
DROP TABLE tll_customer_private.shopify_proofs; DROP SEQUENCE tll_customer_private.shopify_proof_fences;`
  ...
  const probes = [
    ...
    ['browser read-all', 'GRANT pg_read_all_data TO anon WITH INHERIT TRUE', /Unexpected (effective schema authority|private schema CREATE|protected role superuser|effective private data access|effective sequence access)/],
    ['executor read-all', `GRANT pg_read_all_data TO ${executor} WITH INHERIT TRUE`, /Unexpected (sequence authority|effective private table\/column authority|bridge role authority|effective private data access|effective sequence access)/],
    ['browser superuser', 'ALTER ROLE anon SUPERUSER', /Unexpected (effective schema authority|protected role superuser)/],
    ['executor superuser', `ALTER ROLE ${executor} SUPERUSER`, /Unexpected (effective schema authority|bridge role authority|protected role superuser)/],
