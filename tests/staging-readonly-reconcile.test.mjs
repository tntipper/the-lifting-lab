...
test('reconciliation remains disabled and is a single fixed read-only SELECT', () => {
...
test('disabled path reads neither Keychain nor network', async () => {
  ...
  const output = execFileSync(process.execPath,['scripts/staging-readonly-reconcile.mjs'],{encoding:'utf8'}); assert.equal(JSON.parse(output).status,'NATIVE_ACCESS_DISABLED')
})
...
test('manifest pins the disabled sources and fixed query', () => {
  execFileSync(process.execPath,['scripts/staging-readonly-reconcile-manifest.mjs','--check'],{stdio:'pipe'})
