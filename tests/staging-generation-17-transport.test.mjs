...
import { PACKAGE_ID, PROJECT_REF, WINDOW_ID } from '../scripts/staging-generation-17-credentials.mjs'
...
import { executeGeneration17CredentialWindow, GENERATION_17_SOURCE_FINGERPRINT, NATIVE_GENERATION_17_TRANSPORT_ENABLED } from '../scripts/staging-generation-17-transport.mjs'
...
function fixture(fail){const events=[],ports={
  ...
  async dispatchDatabase(){events.push('dispatch');return[{tll_generation_17_credential_receipt:{status:'PASS',packageId:PACKAGE_ID,projectRef:PROJECT_REF,generation:17,windowId:WINDOW_ID,expiresAt:EXPIRES,controlsEnabled:false,runtimeCount:5}}]},
  ...
  return{events,ports}}
const journal=events=>({recordIntent(value){events.push('intent');return{...value,state:'INTENT_RECORDED',runId:'generation-17-test'}},transition(_intent,state){events.push(state);return{state}}})
...
test('generation 17 is disarmed after zero-sessions runtime_sessions_remain recovery with a fixed nonsecret fingerprint',()=>{
  assert.equal(NATIVE_GENERATION_17_TRANSPORT_ENABLED,false);assert.match(GENERATION_17_SOURCE_FINGERPRINT,/^[a-f0-9]{64}$/)
})

test('generation 17 consumed transport contains no native launcher or credential-bearing imports',()=>{
  assert.equal(NATIVE_GENERATION_17_TRANSPORT_ENABLED,false)
  const source=readFileSync('scripts/staging-generation-17-transport.mjs','utf8')
  assert.doesNotMatch(source,/runNativeGeneration17CredentialWindow/)
...
test('generation 17 success stages providers, journals once and verifies before receipt',async()=>{
  const f=fixture(),result=await executeGeneration17CredentialWindow({ports:f.ports,journal:journal(f.events),now:()=>NOW,randomBytes,randomUUID})
  assert.equal(result.status,'CREDENTIALS_VERIFIED_CONTROLS_DISABLED');assert.equal(result.generation, 17);assert.deepEqual(f.events,['preflight','intent','stageVercel','stageSupabase','readback','dispatch','verify','RECEIPT_VALIDATED'])
})

test('a consumed journal is rejected before material, provider, database, recovery, or cleanup work',async()=>{
  const f=fixture(),result=await executeGeneration17CredentialWindow({ports:f.ports,journal:rejectedJournal(f.events),now:()=>NOW,
...
test('generation 17 connection failure recovers once, projects fixed diagnostic and forbids replay',async()=>{
  let byte=0;const f=fixture('verify'),result=await executeGeneration17CredentialWindow({ports:f.ports,journal:journal(f.events),now:()=>NOW,randomBytes:size=>Buffer.alloc(size,++byte),randomUUID:(()=>{let n=0;return()=>`10000000-0000-4000-8000-${String(++n).padStart(12,'0')}`})()})
...
test('generation 17 embeds SCRAM derived from projected passwords into dispatch SQL',async()=>{
  ...
  const f=fixture()
  f.ports.dispatchDatabase=async sql=>{f.events.push('dispatch');capturedSql=sql;return[{tll_generation_17_credential_receipt:{status:'PASS',packageId:PACKAGE_ID,projectRef:PROJECT_REF,generation:17,windowId:WINDOW_ID,expiresAt:EXPIRES,controlsEnabled:false,runtimeCount:5}}]}
  ...
  await executeGeneration17CredentialWindow({ports:f.ports,journal:journal(f.events),now:()=>NOW,randomBytes,randomUUID})
