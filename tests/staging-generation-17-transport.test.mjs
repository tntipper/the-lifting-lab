...
test('generation 17 is armed for one reviewed window with a fixed nonsecret fingerprint',()=>{
  assert.equal(NATIVE_GENERATION_17_TRANSPORT_ENABLED,true);assert.match(GENERATION_17_SOURCE_FINGERPRINT,/^[a-f0-9]{64}$/)
})

test('generation 17 consumed transport contains no native launcher or credential-bearing imports',()=>{
  assert.equal(NATIVE_GENERATION_17_TRANSPORT_ENABLED,true)
