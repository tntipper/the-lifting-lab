...
import {
  ...
  KEEPALIVE_PATH_ENV,
  KEEPALIVE_SCHEMA,
} from '../scripts/staging-generation-17-long-session-contract.mjs'
import { NATIVE_GENERATION_17_TRANSPORT_ENABLED } from '../scripts/staging-generation-17-transport.mjs'
import { NATIVE_GENERATION_17_DATABASE_TRANSPORT_ENABLED } from '../scripts/staging-generation-17-database-transport.mjs'
import { assertSupportedOperatorEntry } from '../scripts/staging-generation-17-run-live-once.mjs'

test('generation 17 native gates are armed for one reviewed window', () => {
  assert.equal(NATIVE_GENERATION_17_TRANSPORT_ENABLED, true)
  assert.equal(NATIVE_GENERATION_17_DATABASE_TRANSPORT_ENABLED, true)
})

test('armed launcher CLI refuses without long-session contract (fail-closed)', () => {
  ...
  assert.match(result.stderr, /LONG_SESSION|long.session|keepalive|TLL_LIVE_LONG_SESSION/i)
})
...
test('assertLongSessionContract accepts valid keepalive held by parent', () => {
  ...
  const result = assertLongSessionContract({
    ...
    ppid: 42,
    readParentComm: () => 'node',
    readParentCmdline: () => 'node scripts/staging-generation-17-run-live-once.mjs',
  })
...
test('journal-watch supports --follow --interval without importing launcher', () => {
  const source = readFileSync('scripts/staging-generation-17-journal-watch.mjs', 'utf8')
  assert.match(source, /--follow/)
  assert.match(source, /--interval/)
  assert.doesNotMatch(source, /runNativeGeneration17CredentialWindow|readSupabaseTokenFromKeychain/)
})

test('live launcher wires long-session contract before armed journal claim', () => {
  const source = readFileSync('scripts/staging-generation-17-live-launcher.mjs', 'utf8')
  assert.match(source, /assertLongSessionContract/)
  assert.match(source, /staging-generation-17-long-session-contract\.mjs/)
