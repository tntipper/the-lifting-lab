...
import {
  ...
  KEEPALIVE_PATH_ENV,
  KEEPALIVE_SCHEMA,
} from '../scripts/staging-generation-19-long-session-contract.mjs'
...
import { assertSupportedOperatorEntry } from '../scripts/staging-generation-19-run-live-once.mjs'

test('generation 19 native gates are armed for one reviewed window', () => {
...
test('assertLongSessionContract accepts valid keepalive held by parent', () => {
  ...
  const result = assertLongSessionContract({
    ...
    ppid: 42,
    readParentComm: () => 'node',
    readParentCmdline: () => 'node scripts/staging-generation-19-run-live-once.mjs',
  })
...
test('journal-watch supports --follow --interval without importing launcher', () => {
  const source = readFileSync('scripts/staging-generation-19-journal-watch.mjs', 'utf8')
  assert.match(source, /--follow/)
  assert.match(source, /--interval/)
  assert.doesNotMatch(source, /runNativeGeneration19CredentialWindow|readSupabaseTokenFromKeychain/)
})

test('live launcher wires long-session contract before armed journal claim', () => {
  const source = readFileSync('scripts/staging-generation-19-live-launcher.mjs', 'utf8')
  assert.match(source, /assertLongSessionContract/)
  assert.match(source, /staging-generation-19-long-session-contract\.mjs/)
