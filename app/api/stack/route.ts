...
async function mutate(request: Request) {
    try {
        ...
        // Session cookies remain authoritative. Every mutation carries its
        // originating account: revisions alone cannot distinguish two users.
        if (request.headers.get('x-stack-expected-user') !== user.id)
            return json({ error: 'Your signed-in account changed. Reconnect before retrying this change.' }, 409);
