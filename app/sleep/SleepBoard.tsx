...
type Filter = 'all' | SleepGroup

const FILTERS: { key: Filter; label: string }[] = [
  { key: 'all', label: 'All' },
  { key: 'foundations', label: GROUP_META.foundations.label },
  { key: 'minerals', label: GROUP_META.minerals.label },
  { key: 'calming', label: GROUP_META.calming.label },
  { key: 'hormonal', label: GROUP_META.hormonal.label },
  { key: 'hyped', label: GROUP_META.hyped.label },
]
...
export default function SleepBoard() {
  ...
  function pickFilter(key: Filter) {
    setFilter(key)
    track('sleep_filter', { filter: key })
  }

  function toggle(item: SleepItem) {
    const next = open === item.id ? null : item.id
    setOpen(next)
    if (next) track('sleep_toggle', { item: item.id, tier: item.tier })
  }
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {FILTERS.map(({ key, label }) => {
          ...
          const count = key === 'all' ? SLEEP_ITEMS.length : itemsInGroup(key).length
          return (
            <button
              key={key}
              type="button"
              ...
              <div className="px-5 pb-5 -mt-1" hidden={!isOpen}>
                ...
                >
                  ...
                  {item.link && (
                    <Link
                      href={item.link.href}
                      onClick={() => track('sleep_cta', { item: item.id, href: item.link!.href })}
