...
type Filter = 'all' | TestGroup

const FILTERS: { key: Filter; label: string }[] = [
  { key: 'all', label: 'All' },
  { key: 'foundations', label: GROUP_META.foundations.label },
  { key: 'nutrients', label: GROUP_META.nutrients.label },
...
export default function TestosteroneBoard() {
  ...
  function pickFilter(key: Filter) {
    setFilter(key)
    track('testosterone_filter', { filter: key })
  }

  function toggle(item: TestItem) {
    const next = open === item.id ? null : item.id
    setOpen(next)
    if (next) track('testosterone_toggle', { item: item.id, tier: item.tier })
  }
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {FILTERS.map(({ key, label }) => {
          ...
          const count = key === 'all' ? TEST_ITEMS.length : itemsInGroup(key).length
          return (
            <button
              key={key}
              type="button"
              ...
              <div className="px-5 pb-5 -mt-1" hidden={!isOpen}>
                ...
                >
                  ...
                  {item.link && (
                    <Link
                      href={item.link.href}
                      onClick={() => track('testosterone_cta', { item: item.id, href: item.link!.href })}
