-- GENERATED by scripts/staging-generation-13-recovery.mjs from the pinned generation-6 recovery; do not edit.
...
BEGIN
  FOREACH r IN ARRAY ARRAY['tll_customer_runtime','tll_cart_runtime','tll_broker_runtime','tll_provisional_runtime','tll_bridge_runtime'] LOOP
    ...
    IF jsonb_typeof(parsed) IS DISTINCT FROM 'object' OR (SELECT count(*) FROM jsonb_object_keys(parsed)) IS DISTINCT FROM 5
      ...
      OR jsonb_typeof(parsed->'generation') IS DISTINCT FROM 'number' OR parsed->>'generation' IS DISTINCT FROM '13'
      OR jsonb_typeof(parsed->'windowId') IS DISTINCT FROM 'string' OR parsed->>'windowId' IS DISTINCT FROM '866b0e78-7530-493a-8963-e8cf24cf3067'
...
BEGIN
  IF current_setting('tll.recovery_idempotent',true)<>'true' THEN
    ...
    marker:='tll-runtime-window/v1 '||jsonb_build_object('expiresAt',current_setting('tll.recovery_expires_at',true),'generation',13,'projectRef','qdmvngjwkcsilzmqksme','state','retired','windowId','866b0e78-7530-493a-8963-e8cf24cf3067')::text;
    FOREACH r IN ARRAY ARRAY['tll_customer_runtime','tll_cart_runtime','tll_broker_runtime','tll_provisional_runtime','tll_bridge_runtime'] LOOP
