-- GENERATED by scripts/staging-generation-20-recovery.mjs from the pinned generation-6 recovery; do not edit.
...
BEGIN
  FOREACH r IN ARRAY ARRAY['tll_customer_runtime','tll_cart_runtime','tll_broker_runtime','tll_provisional_runtime','tll_bridge_runtime'] LOOP
    ...
    END;
    IF parsed->>'state'='active' AND ('2026-09-22T13:23:00.000Z'='UNSET_REQUIRES_REVIEWED_ARMING_DIFF'
      OR parsed->>'expiresAt' IS DISTINCT FROM '2026-09-22T13:23:00.000Z'
      ...
      RAISE EXCEPTION 'Active Gen20 credential drift shape mismatch: %',r;
    ELSIF parsed->>'state'='retired' AND (EXISTS(SELECT 1 FROM pg_roles WHERE rolname=r AND rolcanlogin)
      ...
      RAISE EXCEPTION 'Retired Gen20 credential state is not inert: %',r;
    END IF;
...
BEGIN
  IF current_setting('tll.recovery_idempotent',true)<>'true' THEN
    ...
    marker:='tll-runtime-window/v1 '||jsonb_build_object('expiresAt',current_setting('tll.recovery_expires_at',true),'generation',20,'projectRef','qdmvngjwkcsilzmqksme','state','retired','windowId','a009f2b4-86df-4701-a8bc-1112597e3c42')::text;
    FOREACH r IN ARRAY ARRAY['tll_customer_runtime','tll_cart_runtime','tll_broker_runtime','tll_provisional_runtime','tll_bridge_runtime'] LOOP
