-- GENERATED by scripts/staging-generation-9-recovery.mjs from the pinned generation-6 recovery; do not edit.
...
BEGIN
  FOREACH r IN ARRAY ARRAY['tll_customer_runtime','tll_cart_runtime','tll_broker_runtime','tll_provisional_runtime','tll_bridge_runtime'] LOOP
    ...
    IF jsonb_typeof(parsed) IS DISTINCT FROM 'object' OR (SELECT count(*) FROM jsonb_object_keys(parsed)) IS DISTINCT FROM 5
      ...
      OR jsonb_typeof(parsed->'generation') IS DISTINCT FROM 'number' OR parsed->>'generation' IS DISTINCT FROM '9'
      OR jsonb_typeof(parsed->'windowId') IS DISTINCT FROM 'string' OR parsed->>'windowId' IS DISTINCT FROM '08ceb448-c1e2-4641-85ff-bee536774466'
...
BEGIN
  IF current_setting('tll.recovery_idempotent',true)<>'true' THEN
    ...
    marker:='tll-runtime-window/v1 '||jsonb_build_object('expiresAt',current_setting('tll.recovery_expires_at',true),'generation',9,'projectRef','qdmvngjwkcsilzmqksme','state','retired','windowId','08ceb448-c1e2-4641-85ff-bee536774466')::text;
    FOREACH r IN ARRAY ARRAY['tll_customer_runtime','tll_cart_runtime','tll_broker_runtime','tll_provisional_runtime','tll_bridge_runtime'] LOOP
