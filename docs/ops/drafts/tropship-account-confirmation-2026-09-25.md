...
4. **Accuracy of product information.** Your product pages already show barcodes and nutrition. Are those details checked against the current manufacturer label for the exact flavour and pack, and how are corrections or formulation changes communicated? Some feed nutrition text appears to name a different flavour or product form. If manufacturer label documents or links are available, where can we obtain them? We will write our own descriptions.
