...
The exact one-run command is `node scripts/staging-provider-keychain-fixture-recovery-live-launcher.mjs` with the current working directory set to `implementation-integration`. Its new journal has a 90-second absolute run deadline and shorter phase deadlines; invoke it at most once. The separately verified armed artifact already resides in the fixed private state directory, so use `node scripts/staging-provider-keychain-fixture-recovery-build.mjs --check-armed` after applying the patch; do not try to overwrite it with another build. Invoke the launcher only if the source/receipt/binary hashes, fresh fixture and journals, login Keychain baseline, and exact arming diff all still match. If any check fails, disarm without invoking.
