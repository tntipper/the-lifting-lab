...
- All unapproved historical scores are excluded from effectiveness recommendations, awards and automatic wizard/goal-stack selections. The existing candidate labels some historical numbers as unverified research data; the owner's later opening decision requires neutral or clearly unassessed public products, so numeric legacy displays still need a focused customer-facing containment check before launch. Known category, product, comparison and alternatives pages remain available; score/value order becomes alphabetical and listed-price comparisons remain separate. API ranks are null and metadata follows the same rule. See [assessment approval containment](../research/unassessed-ranking-containment.md); this change does not approve the underlying science.
