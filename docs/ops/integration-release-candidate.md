...
- All unapproved historical scores are excluded from effectiveness recommendations, awards and automatic wizard/goal-stack selections. The local 25 September follow-up also withholds the old numbers from shared public cards, stack/share text and images, comparison hubs and the identified public product APIs; it needs independent cross-surface review and exact deployed acceptance before the opening rule is met. Known category, product, comparison and alternatives pages remain available; score/value order becomes alphabetical and listed-price comparisons remain separate. API ranks are null and metadata follows the same rule. See [assessment approval containment](../research/unassessed-ranking-containment.md); this change does not approve the underlying science.
...
1. Verify the deployed assessment-approval boundary and cache invalidation before release. Keep unassessed comparison products visible without an unapproved public numeric effectiveness score or recommendation. Complete formula, label, claims and category evidence review product by product before restoring any scientific recommendation; no approved assessment dataset currently exists.
