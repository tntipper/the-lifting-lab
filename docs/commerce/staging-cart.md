...
The route does not fetch or provision these credentials. Missing values leave it unavailable. No environment values or provider/SQL error bodies are logged. `migration 006` is an operator-reviewed staging artifact; its control remains false, no LOGIN or runtime membership is provisioned, and it requires explicit staging context and the operator-bound staging marker. The installer retains its existing table/control ownership and one ADMIN-only membership on each of the function owner and gateway, both with INHERIT FALSE / SET FALSE. It can deliberately delegate runtime execution or acquire temporary owner SET/schema CREATE authority for a future reviewed function migration, then retire those temporary grants. At rest it has no effective function-owner or gateway use. This trusted provisioning power does not protect data from the installer itself; see [repository migration authority](../identity/repository-migration-authority.md). Gateway/runtime roles cannot read private tables or change control. Cart's installer table ownership still differs from005's narrower operator-function interface. Applying006 requires `tll.cart_migration_environment=staging` and `tll.cart_expected_project_ref=<approved staging ref>`.
...
`POST` accepts only exact, bounded actions: `{action:"open"}`, `{action:"use_account"}`, or `{action:"transfer",revision}`. Transition actions require a verified account, the guest capability, same-origin intent and the current account-bound CSRF token; transfer also requires a UUID idempotency key. An otherwise valid PATCH-shaped POST returns 400 before authentication, repository or Storefront work.

Every request verifies the current Supabase user. A guest cart is bound to the opaque cookie and guest actor hash. A signed-in cart is bound to a deterministic HMAC session derived server-side from the verified Supabase UUID, so the same account can retrieve it without a cart cookie on another browser. No raw UUID is stored as the cart key or returned in cart responses.

When a customer signs in with a guest cookie, GET exposes `transition_required`; it never reinterprets the guest record as account-owned. The customer must either connect the guest cart or use the cart already saved to the account. Connecting is a one-use custody move with no Storefront mutation and is refused when the account already has a cart. Choosing the account cart clears the browser guest capability without merging or deleting the guest record. The guest cookie is cleared only after the selected outcome is acknowledged. A lost finish response is recovered by inspecting the exact durable transition; no transfer or Storefront write is resent. Sign-out and account switches discard late responses from the previous identity.
...
The browser fixture exercises the actual provider, explicit account transition controls, shared product/manual-stack action, header, cart contents and native dialog. The existing accessibility fixture separately exercises the actual full StackBuilder/FAB and all prior assessment/outbox flows with the cart disabled. It does not claim a combined hosted account/cart journey. Local SQL checks exercise the real PG17 migrations/reservations and role grants; injected connection tests cover explicit COMMIT loss, and provider tests cover create/update/remove/read failures. A hosted Cart API run, exact installed-scopes/version confirmation, dedicated runtime provisioning, migrated-control enablement, actual delivery/tax policy and checkout acceptance are still required before any wider release.
...
The 18 September account-cart unit adds migration 014 and the deterministic account route/UI. Six migration-014 actual-PG cases, four transition adapter/service cases, 32 cart HTTP/runtime cases, and the actual React browser fixture at all six viewports pass. Coverage includes existing-account conflict, explicit no-merge choice, cross-request account retrieval, acknowledged cookie clearing, lost-finish inspection recovery, and no automatic replay. No hosted migration, provider configuration, customer communication, checkout or purchase was performed.
