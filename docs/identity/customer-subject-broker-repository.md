...
Provisional sign-in has no Supabase UUID. Migration retains a pending original UUID/session reservation, with no Auth foreign key, `auth.users`/`auth.identities` mutation or completed identity promotion. A subject maps uniquely to `(shop, issuer, Shopify subject)` and reuses its random `tllb_` identifier. Conflicting target users or a provisional-to-migration claim fail. Sign-in also rejects an existing pending migration: that reservation does not establish that GoTrue linked the original UUID, and exposing its subject to a concurrent sign-in could create another UUID. Until an authoritative promotion state is separately implemented, only provisional reservations support sign-in reuse. A target user cannot reserve two distinct Shopify subjects in this staging shop. Held evidence is not deleted through user deletion or automatic retention.
...
**Validation:** 30 actual PG17 cases and the canonical migration regression pass; 60 focused offline cases (53 protocol and seven adapter), typecheck and focused lint pass.
