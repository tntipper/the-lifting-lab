...
## Disabled transport checkpoint

The reviewed implementation is split across `staging-generation-6-credentials.mjs`, `staging-generation-6-transport.mjs`, the fixed provider/database transports and the connection verifier. Committed native access remains independently disabled in the orchestrator, database transport and Keychain helper. Supabase secret values use an anonymous file descriptor, Vercel values use stdin, and neither transport places a value in argv, environment variables or a file. The exclusive nonsecret journal is created immediately before the sole database dispatch. Any failure after that boundary invokes the pinned recovery transaction once, then its fresh-session proof; an uncertain recovery acknowledgement forbids another request.

The transport generates database passwords, vault keys, the cart HMAC key and broker client secret. It does not recreate or rotate the existing Shopify private Storefront token or confidential Customer Account client secret. Those two values remain explicit provider dependencies and must be retrieved from the existing development-store applications before a usable preview can be deployed.
