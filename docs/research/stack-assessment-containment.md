...
Production release acceptance, live account tests, Safari/iOS and assistive-technology certification remain separate. The browser checks do not prove reload durability for an authenticated addition that is offline or whose server outcome is uncertain; an identity-bound persistent outbox is a separate F09 follow-up. Historical science/provenance remains unapproved, and the future reviewed assessment projection must replace the interim gate across these consumers.
