#!/usr/bin/env node
/**
 * Dedicated Generation 19 live launcher.
 *
 * This is the only module allowed to invoke Gen 19 native live work.
 ...
 *     `scripts/staging-generation-19-run-live-once.mjs` for the whole run
 ...
 * 1. Start via `scripts/staging-generation-19-run-live-once.mjs` in a long-lived process
...
import {
  ...
  PROJECT_REF,
  WINDOW_ID,
} from './staging-generation-19-credentials.mjs'
import {
  executeGeneration19CredentialWindow,
  NATIVE_GENERATION_19_TRANSPORT_ENABLED,
} from './staging-generation-19-transport.mjs'
...
import { assertLongSessionContract } from './staging-generation-19-long-session-contract.mjs'
import {
  persistConnectionFailureEvidence,
  secretFreeLauncherTerminal,
} from './staging-generation-19-connection-failure-evidence.mjs'
import { assertGeneration19PreArmReady } from './staging-generation-19-pre-arm-gate.mjs'
...
export {
  ...
  KEEPALIVE_SCHEMA,
  LONG_SESSION_ENV,
} from './staging-generation-19-long-session-contract.mjs'

export const DEFAULT_PHASE_JOURNAL_PATH = fileURLToPath(
  new URL('../../implementation-state/staging/tll-generation-19-window-phase.json', import.meta.url),
)
...
function gatesArmed() {
  return NATIVE_GENERATION_19_TRANSPORT_ENABLED === true
...
export async function runNativeGeneration19CredentialWindow({
  ...
  // Refuse live native work unless predecessor retirement is proven or reviewed Gen 17 cleanup path is listed.
  await assertGeneration19PreArmReady({ requireArmedGatesFalse: false })
  ...
  try {
    ...
    const [
      { stageVercelSecrets, stageSupabaseSecrets, readbackProviderNames, removeVercelSecrets, removeSupabaseSecrets },
      {
        ...
        dispatchGeneration19Database,
        recoverGeneration19Database,
        verifyGeneration19EntryBaseline,
      ...
      import('./staging-generation-19-database-transport.mjs'),
    ...
    const result = await executeGeneration19CredentialWindow({
      onPhase,
      ports: {
        preflightDatabase: () => verifyGeneration19EntryBaseline({ token }),
        stageVercel: stageVercelSecrets,
        stageSupabase: stageSupabaseSecrets,
        readbackNames: readbackProviderNames,
        dispatchDatabase: sql => dispatchGeneration19Database(sql, { token }),
        verifyConnections: async input => {
          try {
            await verifyGeneration6Connections({ ...input, tlsCa, createRuntime: createStagingPostgresRuntime })
          } catch (error) {
            const projected = Error('Generation-19 connection verification unavailable')
          ...
          try {
            ...
            await verifyGeneration19ZeroSessionsAfterPoolerDrain({ token })
          } catch (error) {
        ...
        recoverDatabase: () => recoverGeneration19Database({ token }),
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    const result = await runNativeGeneration19CredentialWindow()
