/** Disabled generation-10 orchestration over the reviewed shared transports. */
import { createHash } from 'node:crypto'
import { buildGeneration10CredentialSql, createGeneration10DispatchJournal, GENERATION, IDENTITIES, MAX_WINDOW_MS, PACKAGE_ID, PROJECT_REF, WINDOW_ID } from './staging-generation-10-credentials.mjs'
...
export const NATIVE_GENERATION_10_TRANSPORT_ENABLED=false
...
const unavailable=()=>{throw new Error('Generation-10 transport unavailable')}
...
function validateReceipt(rows,expiresAt){
  if(!Array.isArray(rows)||rows.length!==1||Object.keys(rows[0]??{}).join('|')!=='tll_generation_10_credential_receipt')unavailable()
  const value=rows[0].tll_generation_10_credential_receipt,expected={controlsEnabled:false,expiresAt,generation:GENERATION,packageId:PACKAGE_ID,projectRef:PROJECT_REF,runtimeCount:5,status:'PASS',windowId:WINDOW_ID}
...
export async function executeGeneration10CredentialWindow({ports,journal=createGeneration10DispatchJournal(),now=Date.now,randomBytes,randomUUID}={}){
  ...
  try{
    ...
    preflightPassed=true;phase='JOURNAL_INTENT';intent=journal.recordIntent({expiresAt,nowMs})
    phase='MATERIAL_GENERATION';material=generateGeneration6Material({...(randomBytes?{randomBytes}:{}),...(randomUUID?{randomUUID}:{})});projection=projectGeneration6Secrets(material)
    ...
    phase='DATABASE_PACKAGE';const verifiers=Object.fromEntries(purposes.map((purpose,index)=>[purpose,deriveScramVerifier(projection.passwords[purpose],Buffer.alloc(18,index+1))]));const sql=buildGeneration10CredentialSql({expiresAt,verifiers,nowMs})
    dispatchAttempted=true;phase='DATABASE_DISPATCH';const receipt=validateReceipt(await ports.dispatchDatabase(sql),expiresAt)
    ...
    let recovery='NOT_REQUIRED';if(dispatchAttempted){try{await ports.recoverDatabase();recovery='RECOVERY_VERIFIED'}catch{recovery='RECOVERY_REQUIRED'}}
