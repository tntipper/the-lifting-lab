/**
 * Generation 21 long-session start contract (secret-free).
 *
...
export function assertLongSessionContract({
  ...
  if (env[LONG_SESSION_ENV] !== '1') {
    rejectLongSession(
      'FATAL: Generation 21 live launcher refused start — TLL_LIVE_LONG_SESSION=1 is required when native gates are armed. '
      + 'Use scripts/staging-generation-21-run-live-once.mjs in a long-lived process / foreground Shell (block_until ~45–55m). '
  ...
  if (typeof keepalivePath !== 'string' || keepalivePath.length < 1) {
    rejectLongSession(
      'FATAL: Generation 21 live launcher refused start — TLL_LIVE_KEEPALIVE_PATH must point to a keepalive file held open by the parent run-live-once wrapper for the whole run.',
    )
  }
  if (!exists(keepalivePath)) {
    rejectLongSession(
      'FATAL: Generation 21 live launcher refused start — keepalive path is missing. The parent must hold the keepalive open for the entire live window.',
    )
  }
  ...
  try {
    ...
    rejectLongSession(
      'FATAL: Generation 21 live launcher refused start — keepalive path could not be opened for read.',
    )
  } finally {
    if (fd !== undefined) {
  ...
  try {
    ...
    rejectLongSession(
      'FATAL: Generation 21 live launcher refused start — keepalive file is not valid JSON.',
    )
  }
  if (!record || typeof record !== 'object' || record.schema !== KEEPALIVE_SCHEMA) {
    rejectLongSession(
      'FATAL: Generation 21 live launcher refused start — keepalive schema mismatch.',
    )
  }
  if (typeof record.holderPid !== 'number' || !Number.isInteger(record.holderPid) || record.holderPid <= 1) {
    rejectLongSession(
      'FATAL: Generation 21 live launcher refused start — keepalive holderPid is invalid.',
    )
  }
  if (typeof ppid !== 'number' || ppid <= 1) {
    rejectLongSession(
      'FATAL: Generation 21 live launcher refused start — process appears orphaned (ppid<=1). '
  ...
  if (record.holderPid !== ppid) {
    rejectLongSession(
      'FATAL: Generation 21 live launcher refused start — keepalive holderPid does not match process parent. '
      + 'Start only via scripts/staging-generation-21-run-live-once.mjs so the parent holds the keepalive for the whole run.',
    )
  }
  ...
  if ((typeof comm === 'string' && /\bnohup\b/i.test(comm))
    ...
    rejectLongSession(
      'FATAL: Generation 21 live launcher refused start — parent looks like nohup. '
