/**
 * Pure generation-13 credential package and nonsecret dispatch journal.
 *
...
export const GENERATION = 13
export const WINDOW_ID = '866b0e78-7530-493a-8963-e8cf24cf3067'
export const MAX_WINDOW_MS = 60 * 60 * 1000
export const PACKAGE_ID = 'tll-staging-generation-13-credentials/v1'
export const PREDECESSOR = Object.freeze({
  generation: 10,
  windowId: '04e1b5da-b3e1-430b-8247-caf1d46faa5a',
  expiresAt: '2026-09-20T21:08:17.000Z',
})
...
export const DEFAULT_JOURNAL_PATH = fileURLToPath(new URL('../../implementation-state/staging/tll-generation-13-credential-dispatch.json', import.meta.url))

const purposes = Object.keys(IDENTITIES)
const sqlLiteral = value => `'${value.replaceAll("'", "''")}'`
const unavailable = () => { throw new Error('Generation-13 credential package unavailable') }
...
export function buildGeneration13CredentialSql({ expiresAt, verifiers, nowMs = Date.now() }) {
...
BEGIN
 IF current_database()<>'postgres' OR current_user<>'postgres' OR session_user<>'postgres' OR current_user<>session_user
  ...
  RAISE EXCEPTION 'Generation 13 requires exact managed staging postgres operator'; END IF;
 IF to_regclass('tll_staging_private.environment') IS NULL OR (SELECT count(*) FROM tll_staging_private.environment)<>1
  ...
  RAISE EXCEPTION 'Generation 13 staging binding mismatch'; END IF;
 IF (SELECT count(*) FROM pg_roles WHERE rolname IN(${roleList}))<>5
  ...
  RAISE EXCEPTION 'Generation 13 retired predecessor mismatch'; END IF;
 FOREACH r IN ARRAY ARRAY[${logins.map(sqlLiteral).join(',')}] LOOP
  ...
  IF role_marker IS NULL OR role_marker !~ '^tll-runtime-window/v1 [{].*[}]$' THEN
   RAISE EXCEPTION 'Generation 13 retired predecessor marker malformed: %',r; END IF;
  BEGIN parsed_marker:=substring(role_marker FROM '^tll-runtime-window/v1 ([{].*[}])$')::jsonb;
  EXCEPTION WHEN OTHERS THEN RAISE EXCEPTION 'Generation 13 retired predecessor marker invalid: %',r; END;
  IF parsed_marker IS DISTINCT FROM ${sqlLiteral(JSON.stringify({ expiresAt: PREDECESSOR.expiresAt, generation: PREDECESSOR.generation, projectRef: PROJECT_REF, state: 'retired', windowId: PREDECESSOR.windowId }))}::jsonb THEN
   RAISE EXCEPTION 'Generation 13 retired predecessor marker mismatch: %',r; END IF;
 END LOOP;
 IF EXISTS(SELECT FROM pg_auth_members e JOIN pg_roles m ON m.oid=e.member JOIN pg_roles g ON g.oid=e.roleid
  ...
  OR EXISTS(SELECT FROM pg_auth_members e JOIN pg_roles m ON m.oid=e.member JOIN pg_roles g ON g.oid=e.roleid
   WHERE (g.rolname,m.rolname) IN (${expectedPairs})) THEN RAISE EXCEPTION 'Generation 13 runtime memberships are not inert'; END IF;
 IF EXISTS(SELECT FROM pg_stat_activity WHERE backend_type='client backend' AND usename IN(${roleList})) THEN
  RAISE EXCEPTION 'Generation 13 runtime sessions remain'; END IF;
 IF EXISTS(SELECT FROM (VALUES
  ...
  RAISE EXCEPTION 'Generation 13 requires disabled controls'; END IF;
 FOREACH r IN ARRAY ARRAY[${memberships.map(sqlLiteral).join(',')}] LOOP
  IF NOT EXISTS(SELECT FROM pg_auth_members e JOIN pg_roles g ON g.oid=e.roleid WHERE g.rolname=r AND e.member=operator_name::regrole
   AND e.admin_option AND NOT e.inherit_option AND NOT e.set_option) THEN RAISE EXCEPTION 'Generation 13 operator membership mismatch: %',r; END IF;
 END LOOP;
END $preflight$;
${activate}
DO $postflight$
...
BEGIN
 IF (SELECT count(*) FROM pg_roles WHERE rolname IN(${roleList}) AND rolcanlogin AND rolvaliduntil=${expectedValidUntil}::timestamptz
   AND shobj_description(oid,'pg_authid')=expected_marker)<>5 THEN RAISE EXCEPTION 'Generation 13 role state mismatch'; END IF;
 IF EXISTS(SELECT FROM pg_authid WHERE rolname IN(${roleList}) AND rolpassword IS DISTINCT FROM CASE rolname ${expectedPasswordCases} END) THEN
  RAISE EXCEPTION 'Generation 13 verifier mismatch'; END IF;
 IF (SELECT count(*) FROM pg_auth_members e JOIN pg_roles m ON m.oid=e.member JOIN pg_roles g ON g.oid=e.roleid
  ...
  RAISE EXCEPTION 'Generation 13 effective membership mismatch'; END IF;
 IF EXISTS(SELECT FROM (VALUES
   ((SELECT enabled FROM tll_customer_private.control WHERE singleton)),
   ((SELECT enabled FROM tll_cart_private.control WHERE singleton)),
   ((SELECT enabled FROM tll_broker_private.control WHERE singleton)),
   ((SELECT enabled FROM tll_provisional_private.control WHERE singleton)),
   ((SELECT enabled FROM tll_bridge_private.control WHERE singleton))) controls(enabled) WHERE enabled) THEN
  RAISE EXCEPTION 'Generation 13 control changed during install'; END IF;
END $postflight$;
COMMIT;
SELECT jsonb_build_object('status','PASS','packageId','${PACKAGE_ID}','projectRef','${PROJECT_REF}','generation',${GENERATION},'windowId','${WINDOW_ID}','expiresAt',${sqlLiteral(expiresAt)},'controlsEnabled',false,'runtimeCount',5) AS tll_generation_13_credential_receipt;
`
}
...
export function createGeneration13DispatchJournal({ path = DEFAULT_JOURNAL_PATH, fileSystem = fs, makeRunId = randomUUID } = {}) {
  let ownedRunId
