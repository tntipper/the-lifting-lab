#!/usr/bin/env node
/**
 * Dedicated Generation 21 live launcher.
 *
 * This is the only module allowed to invoke Gen 21 native live work.
 ...
 *     `scripts/staging-generation-21-run-live-once.mjs` for the whole run
 ...
 * 1. Start via `scripts/staging-generation-21-run-live-once.mjs` in a long-lived process
...
import {
  ...
  PROJECT_REF,
  WINDOW_ID,
} from './staging-generation-21-credentials.mjs'
import {
  executeGeneration21CredentialWindow,
  NATIVE_GENERATION_21_TRANSPORT_ENABLED,
} from './staging-generation-21-transport.mjs'
...
import { assertLongSessionContract } from './staging-generation-21-long-session-contract.mjs'
import {
  persistConnectionFailureEvidence,
  secretFreeLauncherTerminal,
} from './staging-generation-21-connection-failure-evidence.mjs'
import { assertGeneration21PreArmReady, loadRetirementEvidenceFromPath, RETIREMENT_EVIDENCE_PATH_ENV } from './staging-generation-21-pre-arm-gate.mjs'
...
export const DEFAULT_PHASE_JOURNAL_PATH = fileURLToPath(
  new URL('../../implementation-state/staging/tll-generation-21-window-phase.json', import.meta.url),
)
...
function gatesArmed() {
  return NATIVE_GENERATION_21_TRANSPORT_ENABLED === true
...
export async function runNativeGeneration21CredentialWindow({
  ...
  const evidence = retirementEvidence === undefined
    ...
    : retirementEvidence
  await assertGeneration21PreArmReady({ retirementEvidence: evidence, requireArmedGatesFalse: false })
  ...
  try {
    ...
    const [
      { stageVercelSecrets, stageSupabaseSecrets, readbackProviderNames, removeVercelSecrets, removeSupabaseSecrets },
      {
        ...
        dispatchGeneration21Database,
        recoverGeneration21Database,
        verifyGeneration21EntryBaseline,
      ...
      import('./staging-generation-21-database-transport.mjs'),
    ...
    const result = await executeGeneration21CredentialWindow({
      onPhase,
      ports: {
        preflightDatabase: () => verifyGeneration21EntryBaseline({ token }),
        stageVercel: stageVercelSecrets,
        stageSupabase: stageSupabaseSecrets,
        readbackNames: readbackProviderNames,
        dispatchDatabase: sql => dispatchGeneration21Database(sql, { token }),
        verifyConnections: async input => {
          try {
            await verifyGeneration6Connections({ ...input, tlsCa, createRuntime: createStagingPostgresRuntime })
          } catch (error) {
            const projected = Error('Generation-21 connection verification unavailable')
          ...
          try {
            ...
            await verifyGeneration21ZeroSessionsAfterPoolerDrain({ token })
          } catch (error) {
        ...
        recoverDatabase: () => recoverGeneration21Database({ token }),
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    const result = await runNativeGeneration21CredentialWindow()
