...
export function readSupabaseTokenFromKeychain(){
  if(!NATIVE_GENERATION_16_DATABASE_TRANSPORT_ENABLED||process.platform!=='darwin')unavailable()
  const helper=fileURLToPath(new URL('./staging-generation-16-keychain.py',import.meta.url))
...
export async function dispatchGeneration16Database(sql,{token,post=postManagementQuery}={}){
  if(typeof sql!=='string'||!sql.startsWith('BEGIN;\n')||!sql.endsWith(' AS tll_generation_16_credential_receipt;\n')
    ||sql.split('tll_generation_16_credential_receipt').length!==2||!sql.includes(PACKAGE_ID)||!sql.includes(WINDOW_ID))unavailable()
...
export const GENERATION_16_ENTRY_BASELINE_SQL=`BEGIN READ ONLY;
...
DO $preflight$ DECLARE r text; role_marker text; parsed_marker jsonb; BEGIN
 IF current_database()<>'postgres' OR current_user<>'postgres' OR session_user<>'postgres'
  ...
  RAISE EXCEPTION 'Generation 16 entry operator mismatch'; END IF;
 IF NOT EXISTS(SELECT FROM tll_staging_private.environment WHERE singleton AND environment='tll-hosted-staging-v1'
  AND operator_project_ref='${PROJECT_REF}') THEN RAISE EXCEPTION 'Generation 16 entry environment mismatch'; END IF;
 IF (SELECT count(*) FROM pg_roles WHERE rolname IN ('tll_customer_runtime','tll_cart_runtime','tll_broker_runtime','tll_provisional_runtime','tll_bridge_runtime'))<>5
  ...
  RAISE EXCEPTION 'Generation 16 entry predecessor mismatch'; END IF;
 FOREACH r IN ARRAY ARRAY['tll_customer_runtime','tll_cart_runtime','tll_broker_runtime','tll_provisional_runtime','tll_bridge_runtime'] LOOP
  ...
  IF role_marker IS NULL OR role_marker !~ '^tll-runtime-window/v1 [{].*[}]$' THEN RAISE EXCEPTION 'Generation 16 entry predecessor marker malformed: %',r; END IF;
  BEGIN parsed_marker:=substring(role_marker FROM '^tll-runtime-window/v1 ([{].*[}])$')::jsonb;
  EXCEPTION WHEN OTHERS THEN RAISE EXCEPTION 'Generation 16 entry predecessor marker invalid: %',r; END;
  IF parsed_marker IS DISTINCT FROM '${predecessorMarkerPayload}'::jsonb THEN RAISE EXCEPTION 'Generation 16 entry predecessor marker mismatch: %',r; END IF;
 END LOOP;
 IF EXISTS(SELECT FROM (VALUES ((SELECT enabled FROM tll_customer_private.control WHERE singleton)),((SELECT enabled FROM tll_cart_private.control WHERE singleton)),((SELECT enabled FROM tll_broker_private.control WHERE singleton)),((SELECT enabled FROM tll_provisional_private.control WHERE singleton)),((SELECT enabled FROM tll_bridge_private.control WHERE singleton))) controls(enabled) WHERE enabled) THEN
  RAISE EXCEPTION 'Generation 16 entry control enabled'; END IF;
END $preflight$;
COMMIT;
SELECT jsonb_build_object('status','ENTRY_BASELINE_PASS','projectRef','${PROJECT_REF}','windowId','${WINDOW_ID}','runtimeGeneration',15,'runtimeInert',true,'controlsEnabled',false) AS tll_generation_16_entry_baseline;
`
export async function verifyGeneration16EntryBaseline({token,post=postManagementQuery}={}){
  const rows=await post(token,GENERATION_16_ENTRY_BASELINE_SQL)
  if(!Array.isArray(rows)||rows.length!==1||Object.keys(rows[0]??{}).join('|')!=='tll_generation_16_entry_baseline')unavailable()
  const value=rows[0].tll_generation_16_entry_baseline,expected={controlsEnabled:false,projectRef:PROJECT_REF,runtimeGeneration:15,runtimeInert:true,status:'ENTRY_BASELINE_PASS',windowId:WINDOW_ID}
...
SET LOCAL statement_timeout='15s';
DO $verify$ BEGIN
 IF EXISTS(SELECT FROM pg_stat_activity WHERE backend_type='client backend' AND usename IN ('tll_customer_runtime','tll_cart_runtime','tll_broker_runtime','tll_provisional_runtime','tll_bridge_runtime')) THEN RAISE EXCEPTION 'Generation 16 runtime sessions remain'; END IF;
 IF EXISTS(SELECT FROM (VALUES ((SELECT enabled FROM tll_customer_private.control WHERE singleton)),((SELECT enabled FROM tll_cart_private.control WHERE singleton)),((SELECT enabled FROM tll_broker_private.control WHERE singleton)),((SELECT enabled FROM tll_provisional_private.control WHERE singleton)),((SELECT enabled FROM tll_bridge_private.control WHERE singleton))) controls(enabled) WHERE enabled) THEN RAISE EXCEPTION 'Generation 16 control enabled during zero-session proof'; END IF;
END $verify$;
COMMIT;
SELECT jsonb_build_object('status','ZERO_SESSIONS','projectRef','${PROJECT_REF}','windowId','${WINDOW_ID}','controlsEnabled',false) AS tll_generation_16_zero_sessions;
`
export async function verifyGeneration16ZeroSessions({token,post=postManagementQuery}={}){
  const rows=await post(token,zeroSessionSql)
  const receiptMismatch=()=>{throw new Error('Generation-16 zero-session receipt mismatch')}
  if(!Array.isArray(rows)||rows.length!==1||Object.keys(rows[0]??{}).join('|')!=='tll_generation_16_zero_sessions')receiptMismatch()
  const value=rows[0].tll_generation_16_zero_sessions,expected={controlsEnabled:false,projectRef:PROJECT_REF,status:'ZERO_SESSIONS',windowId:WINDOW_ID}
...
function projectZeroSessionsFailure(error,failureReason){
  const projected=Error('Generation-16 zero-session verification unavailable')
 ...
 */
export async function verifyGeneration16ZeroSessionsAfterPoolerDrain({
  token,
  ...
  for(let attempt=1;attempt<=maxAttempts;attempt+=1){
    try{
      return await verifyGeneration16ZeroSessions({token,post})
    }catch(error){
      lastError=error
...
const recoverySql=()=>readFileSync(new URL('../config/staging-generation-16-recovery.sql',import.meta.url),'utf8')+
  `\nSELECT jsonb_build_object('status','RECOVERY_COMMITTED','projectRef','${PROJECT_REF}','windowId','${WINDOW_ID}') AS tll_generation_16_recovery_receipt;\n`
const postCommitSql=()=>readFileSync(new URL('../config/staging-generation-16-recovery-postcommit.sql',import.meta.url),'utf8')+
  `\nSELECT jsonb_build_object('status','RECOVERY_VERIFIED','projectRef','${PROJECT_REF}','windowId','${WINDOW_ID}') AS tll_generation_16_recovery_postcommit;\n`
...
export async function recoverGeneration16Database({token,post=postManagementQuery}={}){
  exact(await post(token,recoverySql()),'tll_generation_16_recovery_receipt','RECOVERY_COMMITTED')
  exact(await post(token,postCommitSql()),'tll_generation_16_recovery_postcommit','RECOVERY_VERIFIED')
