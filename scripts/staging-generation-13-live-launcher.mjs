#!/usr/bin/env node
/**
 * Dedicated Generation 13 live launcher.
 *
 * This is the only module allowed to invoke Gen 13 native live work.
 ...
 * 1. Start via `scripts/staging-generation-13-run-live-once.mjs` in a long-lived process
 ...
 * 5. Never replay Generation 11 or Generation 12 (`INTENT_RECORDED` locks) or
 *    this window once consumed.
 */
...
import {
  ...
  PROJECT_REF,
  WINDOW_ID,
} from './staging-generation-13-credentials.mjs'
import {
  executeGeneration13CredentialWindow,
  NATIVE_GENERATION_13_TRANSPORT_ENABLED,
} from './staging-generation-13-transport.mjs'
...
function gatesArmed() {
  return NATIVE_GENERATION_13_TRANSPORT_ENABLED === true
    && NATIVE_GENERATION_13_DATABASE_TRANSPORT_ENABLED === true
}

/** Fail-closed native entry. Returns disabled status unless every gate is armed. */
export async function runNativeGeneration13CredentialWindow({
  ...
  try {
    ...
    const [
      { stageVercelSecrets, stageSupabaseSecrets, readbackProviderNames, removeVercelSecrets, removeSupabaseSecrets },
      {
        ...
        dispatchGeneration13Database,
        recoverGeneration13Database,
        verifyGeneration13EntryBaseline,
        verifyGeneration13ZeroSessions,
      },
      ...
      import('./staging-generation-13-database-transport.mjs'),
    ...
    const result = await executeGeneration13CredentialWindow({
      onPhase,
      ports: {
        preflightDatabase: () => verifyGeneration13EntryBaseline({ token }),
        stageVercel: stageVercelSecrets,
        stageSupabase: stageSupabaseSecrets,
        readbackNames: readbackProviderNames,
        dispatchDatabase: sql => dispatchGeneration13Database(sql, { token }),
        verifyConnections: async input => {
          try {
            await verifyGeneration6Connections({ ...input, tlsCa, createRuntime: createStagingPostgresRuntime })
          } catch (error) {
            const projected = Error('Generation-13 connection verification unavailable')
            projected.connectionFailure = connectionFailureReport(error)
            throw projected
          }
          await verifyGeneration13ZeroSessions({ token })
        },
        recoverDatabase: () => recoverGeneration13Database({ token }),
