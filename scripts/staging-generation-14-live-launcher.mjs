#!/usr/bin/env node
/**
 * Dedicated Generation 14 live launcher.
 *
 * This is the only module allowed to invoke Gen 14 native live work.
 ...
 * 1. Start via `scripts/staging-generation-14-run-live-once.mjs` in a long-lived process
...
import {
  ...
  PROJECT_REF,
  WINDOW_ID,
} from './staging-generation-14-credentials.mjs'
import {
  executeGeneration14CredentialWindow,
  NATIVE_GENERATION_14_TRANSPORT_ENABLED,
} from './staging-generation-14-transport.mjs'
import { NATIVE_GENERATION_14_DATABASE_TRANSPORT_ENABLED } from './staging-generation-14-database-transport.mjs'
import { createStagingWindowPhaseJournal } from './staging-window-phase-journal.mjs'
import { assertLongSessionContract } from './staging-generation-14-long-session-contract.mjs'
import {
  persistConnectionFailureEvidence,
  secretFreeLauncherTerminal,
} from './staging-generation-14-connection-failure-evidence.mjs'
...
export {
  ...
  KEEPALIVE_SCHEMA,
  LONG_SESSION_ENV,
} from './staging-generation-14-long-session-contract.mjs'

export const DEFAULT_PHASE_JOURNAL_PATH = fileURLToPath(
  new URL('../../implementation-state/staging/tll-generation-14-window-phase.json', import.meta.url),
)
...
function gatesArmed() {
  return NATIVE_GENERATION_14_TRANSPORT_ENABLED === true
    && NATIVE_GENERATION_14_DATABASE_TRANSPORT_ENABLED === true
}

/** Fail-closed native entry. Returns disabled status unless every gate is armed. */
export async function runNativeGeneration14CredentialWindow({
  ...
  try {
    ...
    const [
      { stageVercelSecrets, stageSupabaseSecrets, readbackProviderNames, removeVercelSecrets, removeSupabaseSecrets },
      {
        ...
        dispatchGeneration14Database,
        recoverGeneration14Database,
        verifyGeneration14EntryBaseline,
        verifyGeneration14ZeroSessions,
      },
      ...
      import('./staging-generation-14-database-transport.mjs'),
    ...
    const result = await executeGeneration14CredentialWindow({
      onPhase,
      ports: {
        preflightDatabase: () => verifyGeneration14EntryBaseline({ token }),
        stageVercel: stageVercelSecrets,
        stageSupabase: stageSupabaseSecrets,
        readbackNames: readbackProviderNames,
        dispatchDatabase: sql => dispatchGeneration14Database(sql, { token }),
        verifyConnections: async input => {
          try {
            await verifyGeneration6Connections({ ...input, tlsCa, createRuntime: createStagingPostgresRuntime })
          } catch (error) {
            const projected = Error('Generation-14 connection verification unavailable')
            projected.connectionFailure = connectionFailureReport(error)
            throw projected
          }
          await verifyGeneration14ZeroSessions({ token })
        },
        recoverDatabase: () => recoverGeneration14Database({ token }),
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    const result = await runNativeGeneration14CredentialWindow()
