#!/usr/bin/env node
/**
 * Dedicated Generation 16 live launcher.
 *
 ...
 *     `scripts/staging-generation-16-run-live-once.mjs` for the whole run
 ...
 * 1. Start via `scripts/staging-generation-16-run-live-once.mjs` in a long-lived process
...
import {
  ...
  PROJECT_REF,
  WINDOW_ID,
} from './staging-generation-16-credentials.mjs'
import {
  executeGeneration16CredentialWindow,
  NATIVE_GENERATION_16_TRANSPORT_ENABLED,
} from './staging-generation-16-transport.mjs'
import { NATIVE_GENERATION_16_DATABASE_TRANSPORT_ENABLED } from './staging-generation-16-database-transport.mjs'
import { createStagingWindowPhaseJournal } from './staging-window-phase-journal.mjs'
import { assertLongSessionContract } from './staging-generation-16-long-session-contract.mjs'
import {
  persistConnectionFailureEvidence,
  secretFreeLauncherTerminal,
} from './staging-generation-16-connection-failure-evidence.mjs'
...
export {
  ...
  KEEPALIVE_SCHEMA,
  LONG_SESSION_ENV,
} from './staging-generation-16-long-session-contract.mjs'

export const DEFAULT_PHASE_JOURNAL_PATH = fileURLToPath(
  new URL('../../implementation-state/staging/tll-generation-16-window-phase.json', import.meta.url),
)
...
function gatesArmed() {
  return NATIVE_GENERATION_16_TRANSPORT_ENABLED === true
    && NATIVE_GENERATION_16_DATABASE_TRANSPORT_ENABLED === true
}

/** Fail-closed native entry. Returns disabled status unless every gate is armed. */
export async function runNativeGeneration16CredentialWindow({
  ...
  try {
    ...
    const [
      { stageVercelSecrets, stageSupabaseSecrets, readbackProviderNames, removeVercelSecrets, removeSupabaseSecrets },
      {
        ...
        dispatchGeneration16Database,
        recoverGeneration16Database,
        verifyGeneration16EntryBaseline,
      ...
      import('./staging-generation-16-database-transport.mjs'),
    ...
    const result = await executeGeneration16CredentialWindow({
      onPhase,
      ports: {
        preflightDatabase: () => verifyGeneration16EntryBaseline({ token }),
        stageVercel: stageVercelSecrets,
        stageSupabase: stageSupabaseSecrets,
        readbackNames: readbackProviderNames,
        dispatchDatabase: sql => dispatchGeneration16Database(sql, { token }),
        verifyConnections: async input => {
          try {
            await verifyGeneration6Connections({ ...input, tlsCa, createRuntime: createStagingPostgresRuntime })
          } catch (error) {
            const projected = Error('Generation-16 connection verification unavailable')
          ...
          try {
            ...
            await verifyGeneration16ZeroSessionsAfterPoolerDrain({ token })
          } catch (error) {
        ...
        recoverDatabase: () => recoverGeneration16Database({ token }),
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    const result = await runNativeGeneration16CredentialWindow()
