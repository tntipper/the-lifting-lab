...
const unavailable=()=>{throw new Error('Generation-7 database transport unavailable')}
...
export function readSupabaseTokenFromKeychain(){
  if(!NATIVE_GENERATION_7_DATABASE_TRANSPORT_ENABLED||process.platform!=='darwin')unavailable()
  const helper=fileURLToPath(new URL('./staging-generation-7-keychain.py',import.meta.url))
...
export async function postManagementQuery(token,query,{request=https.request}={}){
  ...
  try{return await new Promise((resolve,reject)=>{let req,done=false,size=0;const chunks=[]
    ...
    const fail=()=>{try{req?.destroy()}catch{};finish(new Error('Generation-7 database transport unavailable'))};const timer=setTimeout(fail,TIMEOUT_MS)
...
export async function dispatchGeneration7Database(sql,{token,post=postManagementQuery}={}){
  if(typeof sql!=='string'||!sql.startsWith('BEGIN;\n')||!sql.endsWith(' AS tll_generation_7_credential_receipt;\n')
    ||sql.split('tll_generation_7_credential_receipt').length!==2||!sql.includes(PACKAGE_ID)||!sql.includes(WINDOW_ID))unavailable()
...
DO $preflight$ BEGIN
 IF current_database()<>'postgres' OR current_user<>'postgres' OR session_user<>'postgres'
  ...
  RAISE EXCEPTION 'Generation 7 entry operator mismatch'; END IF;
 IF NOT EXISTS(SELECT FROM tll_staging_private.environment WHERE singleton AND environment='tll-hosted-staging-v1'
  AND operator_project_ref='${PROJECT_REF}') THEN RAISE EXCEPTION 'Generation 7 entry environment mismatch'; END IF;
...
END $preflight$;
COMMIT;
SELECT jsonb_build_object('status','ENTRY_BASELINE_PASS','projectRef','${PROJECT_REF}','windowId','${WINDOW_ID}','runtimeGeneration',6,'runtimeInert',true,'controlsEnabled',false) AS tll_generation_7_entry_baseline;
`
export async function verifyGeneration7EntryBaseline({token,post=postManagementQuery}={}){
  const rows=await post(token,GENERATION_7_ENTRY_BASELINE_SQL)
  if(!Array.isArray(rows)||rows.length!==1||Object.keys(rows[0]??{}).join('|')!=='tll_generation_7_entry_baseline')unavailable()
  const value=rows[0].tll_generation_7_entry_baseline,expected={controlsEnabled:false,projectRef:PROJECT_REF,runtimeGeneration:6,runtimeInert:true,status:'ENTRY_BASELINE_PASS',windowId:WINDOW_ID}
...
SET LOCAL statement_timeout='15s';
DO $verify$ BEGIN
 IF EXISTS(SELECT FROM pg_stat_activity WHERE backend_type='client backend' AND usename IN ('tll_customer_runtime','tll_cart_runtime','tll_broker_runtime','tll_provisional_runtime','tll_bridge_runtime')) THEN RAISE EXCEPTION 'Generation 7 runtime sessions remain'; END IF;
 IF EXISTS(SELECT FROM (VALUES ((SELECT enabled FROM tll_customer_private.control WHERE singleton)),((SELECT enabled FROM tll_cart_private.control WHERE singleton)),((SELECT enabled FROM tll_broker_private.control WHERE singleton)),((SELECT enabled FROM tll_provisional_private.control WHERE singleton)),((SELECT enabled FROM tll_bridge_private.control WHERE singleton))) controls(enabled) WHERE enabled) THEN RAISE EXCEPTION 'Generation 7 control enabled during zero-session proof'; END IF;
END $verify$;
COMMIT;
SELECT jsonb_build_object('status','ZERO_SESSIONS','projectRef','${PROJECT_REF}','windowId','${WINDOW_ID}','controlsEnabled',false) AS tll_generation_7_zero_sessions;
`
export async function verifyGeneration7ZeroSessions({token,post=postManagementQuery}={}){
  const rows=await post(token,zeroSessionSql)
  if(!Array.isArray(rows)||rows.length!==1||Object.keys(rows[0]??{}).join('|')!=='tll_generation_7_zero_sessions')unavailable()
  const value=rows[0].tll_generation_7_zero_sessions,expected={controlsEnabled:false,projectRef:PROJECT_REF,status:'ZERO_SESSIONS',windowId:WINDOW_ID}
  if(!value||Object.keys(value).sort().join('|')!==Object.keys(expected).sort().join('|'))unavailable()
  for(const [key,wanted]of Object.entries(expected))if(value[key]!==wanted)unavailable()
  return Object.freeze(expected)
}

const recoverySql=()=>readFileSync(new URL('../config/staging-generation-7-recovery.sql',import.meta.url),'utf8')+
  `\nSELECT jsonb_build_object('status','RECOVERY_COMMITTED','projectRef','${PROJECT_REF}','windowId','${WINDOW_ID}') AS tll_generation_7_recovery_receipt;\n`
const postCommitSql=()=>readFileSync(new URL('../config/staging-generation-7-recovery-postcommit.sql',import.meta.url),'utf8')+
  `\nSELECT jsonb_build_object('status','RECOVERY_VERIFIED','projectRef','${PROJECT_REF}','windowId','${WINDOW_ID}') AS tll_generation_7_recovery_postcommit;\n`
...
export async function recoverGeneration7Database({token,post=postManagementQuery}={}){
  exact(await post(token,recoverySql()),'tll_generation_7_recovery_receipt','RECOVERY_COMMITTED')
  exact(await post(token,postCommitSql()),'tll_generation_7_recovery_postcommit','RECOVERY_VERIFIED')
