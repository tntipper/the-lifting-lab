...
export async function executeGeneration8CredentialWindow({ports,journal=createGeneration8DispatchJournal(),now=Date.now,randomBytes,randomUUID}={}){
  ...
  try{
    ...
    phase='DATABASE_PACKAGE';const verifiers=Object.fromEntries(purposes.map((purpose,index)=>[purpose,deriveScramVerifier(projection.passwords[purpose],Buffer.alloc(18,index+1))]));const sql=buildGeneration8CredentialSql({expiresAt,verifiers,nowMs})
