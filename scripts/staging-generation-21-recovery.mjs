...
const successor=Object.freeze({generation: 21,windowId:'a5511645-77af-4fc9-9e4c-f5c8a474d5fa'})
// No Gen21 credential window exists in Phase 1. A reviewed arming diff must
// replace this sentinel with that window's exact bounded expiry before recovery
// can run. When armed, active roles must use that same expiry as VALID UNTIL;
// `infinity` is rejected so the recovery model matches the credential installer.
const activeExpiresAt=ACTIVE_WINDOW_EXPIRES_AT
const files=Object.freeze([
  Object.freeze({source:'config/staging-account-activation-recovery.sql',sourceSha256:'69b6cae3da32c62512008e14c3fb6c68746e9ee533a1dedcaa690ff349fea87c',output:'config/staging-generation-21-recovery.sql'}),
  Object.freeze({source:'config/staging-account-activation-recovery-postcommit.sql',sourceSha256:'278953bd725c9ccdba2e2da0faaafac76aafd94e65234e1fa0c62d68b1d78d35',output:'config/staging-generation-21-recovery-postcommit.sql'}),
])
const unavailable=()=>{throw new Error('Generation-21 recovery generation unavailable')}
...
function transform(value){
  ...
  value=replaceRequired(value,'-- GENERATED by scripts/staging-account-activation-recovery.mjs; do not edit.','-- GENERATED by scripts/staging-generation-21-recovery.mjs from the pinned generation-6 recovery; do not edit.')
  ...
  if(!isPostCommit){
    value=replaceRequired(value,
      ...
      RAISE EXCEPTION 'Runtime marker expiry is invalid: %',r;
    END;
    IF parsed->>'state'='active' AND ('${activeExpiresAt}'='UNSET_REQUIRES_REVIEWED_ARMING_DIFF'
      OR parsed->>'expiresAt' IS DISTINCT FROM '${activeExpiresAt}'
  ...
  if(!isPostCommit)value=replaceRequired(value,
...
SELECT jsonb_build_object('queryId','tll-staging-generation-21-retirement-postcommit/v1','projectRef','qdmvngjwkcsilzmqksme','generation',21,'windowId','${successor.windowId}','status','PASS_RETIRED','runtimeRoles',5,'runtimeSessions',0,'controlsEnabled',0,'passwordsConfigured',0,'validUntilInfinity',5,'executionEdges',0,'operatorEdges',5) AS tll_gen21_retirement_postcommit;
`)
  }
...
if(process.argv.includes('--check')){
  ...
  if(stale){console.error('generation-21 recovery artifact is stale');process.exitCode=1}
