/** One-use, secret-free phase record for the V1 synthetic fixture cleanup only. */
import { randomUUID } from 'node:crypto'
import * as fs from 'node:fs'
import { dirname, join } from 'node:path'

const SCHEMA = 'tll-stage3-fixture-recovery/v1'
const PHASES = Object.freeze(['PREFLIGHT', 'API_DELETE', 'SIDECAR_RECONCILE', 'DIRECTORY_REMOVE', 'FINAL_RECONCILIATION'])
const PHASE_DEADLINES_MS = Object.freeze({ PREFLIGHT: 15_000, API_DELETE: 30_000,
  SIDECAR_RECONCILE: 15_000, DIRECTORY_REMOVE: 10_000, FINAL_RECONCILIATION: 10_000 })
const RUN_DEADLINE_MS = 90_000
const OUTCOMES = Object.freeze(['PASS', 'HOLD', 'UNCERTAIN'])
const fail = () => { throw Error('Synthetic Keychain fixture recovery journal unavailable') }
...
function parse(path, io) {
  ...
  if (!exact(record, ['schema', 'runId', 'sourceSha256', 'binarySha256',
    ...
    || [record.sourceSha256, record.binarySha256]
...
function durableWrite(path, record, io, { initial = false } = {}) {
  ...
  const temporary = initial ? path : join(dirname(path), `.tll-fixture-recovery-${record.runId}-${record.sequence}.tmp`)
  let descriptor
  try {
...
export function createFixtureRecoveryJournal({ path, io = fs, now = Date.now, makeRunId = randomUUID } = {}) {
  ...
  return Object.freeze({
    ...
    start(identity) {
      ...
      if (!exact(identity, ['sourceSha256', 'binarySha256'])
    ...
    advance(previous, phase) {
      if (!previous || previous.runId !== ownedId || previous.outcome !== null
        ...
        || PHASES.indexOf(phase) !== PHASES.indexOf(previous.phase) + 1) fail()
      ...
      if (!Number.isFinite(updatedMs) || updatedMs < Date.parse(current.updatedAt)
        || updatedMs >= Date.parse(current.phaseDeadlineAt)
