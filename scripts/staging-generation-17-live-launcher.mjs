#!/usr/bin/env node
/**
 * Dedicated Generation 17 live launcher.
 *
 ...
 *     `scripts/staging-generation-17-run-live-once.mjs` for the whole run
 ...
 * 1. Start via `scripts/staging-generation-17-run-live-once.mjs` in a long-lived process
...
import {
  ...
  PROJECT_REF,
  WINDOW_ID,
} from './staging-generation-17-credentials.mjs'
import {
  executeGeneration17CredentialWindow,
  NATIVE_GENERATION_17_TRANSPORT_ENABLED,
} from './staging-generation-17-transport.mjs'
import { NATIVE_GENERATION_17_DATABASE_TRANSPORT_ENABLED } from './staging-generation-17-database-transport.mjs'
import { createStagingWindowPhaseJournal } from './staging-window-phase-journal.mjs'
import { assertLongSessionContract } from './staging-generation-17-long-session-contract.mjs'
import {
  persistConnectionFailureEvidence,
  secretFreeLauncherTerminal,
} from './staging-generation-17-connection-failure-evidence.mjs'
...
export {
  ...
  KEEPALIVE_SCHEMA,
  LONG_SESSION_ENV,
} from './staging-generation-17-long-session-contract.mjs'

export const DEFAULT_PHASE_JOURNAL_PATH = fileURLToPath(
  new URL('../../implementation-state/staging/tll-generation-17-window-phase.json', import.meta.url),
)
...
function gatesArmed() {
  return NATIVE_GENERATION_17_TRANSPORT_ENABLED === true
    && NATIVE_GENERATION_17_DATABASE_TRANSPORT_ENABLED === true
}

/** Fail-closed native entry. Returns disabled status unless every gate is armed. */
export async function runNativeGeneration17CredentialWindow({
  ...
  try {
    ...
    const [
      { stageVercelSecrets, stageSupabaseSecrets, readbackProviderNames, removeVercelSecrets, removeSupabaseSecrets },
      {
        ...
        dispatchGeneration17Database,
        recoverGeneration17Database,
        verifyGeneration17EntryBaseline,
      ...
      import('./staging-generation-17-database-transport.mjs'),
    ...
    const result = await executeGeneration17CredentialWindow({
      onPhase,
      ports: {
        preflightDatabase: () => verifyGeneration17EntryBaseline({ token }),
        stageVercel: stageVercelSecrets,
        stageSupabase: stageSupabaseSecrets,
        readbackNames: readbackProviderNames,
        dispatchDatabase: sql => dispatchGeneration17Database(sql, { token }),
        verifyConnections: async input => {
          try {
            await verifyGeneration6Connections({ ...input, tlsCa, createRuntime: createStagingPostgresRuntime })
          } catch (error) {
            const projected = Error('Generation-17 connection verification unavailable')
        ...
        recoverDatabase: () => recoverGeneration17Database({ token }),
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    const result = await runNativeGeneration17CredentialWindow()
