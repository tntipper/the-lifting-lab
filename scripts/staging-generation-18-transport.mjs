/** Disabled generation-18 orchestration over the reviewed shared transports. */
...
const unavailable=()=>{throw new Error('Generation-18 transport unavailable')}
...
function validateReceipt(rows,expiresAt){
  if(!Array.isArray(rows)||rows.length!==1||Object.keys(rows[0]??{}).join('|')!=='tll_generation_18_credential_receipt')unavailable()
  const value=rows[0].tll_generation_18_credential_receipt,expected={controlsEnabled:false,expiresAt,generation:GENERATION,packageId:PACKAGE_ID,projectRef:PROJECT_REF,runtimeCount:5,status:'PASS',windowId:WINDOW_ID}
...
export async function executeGeneration18CredentialWindow({ports,journal=createGeneration18DispatchJournal(),now=Date.now,randomBytes,randomUUID,onPhase}={}){
  ...
  try{
    ...
    await notify('DATABASE_PACKAGE');const verifiers=Object.fromEntries(purposes.map((purpose,index)=>[purpose,deriveScramVerifier(projection.passwords[purpose],Buffer.alloc(18,index+1))]));const sql=buildGeneration18CredentialSql({expiresAt,verifiers,nowMs})
...
export const GENERATION_18_SOURCE_FINGERPRINT=createHash('sha256').update(`${PACKAGE_ID}|${WINDOW_ID}|${GENERATION}`).digest('hex')
