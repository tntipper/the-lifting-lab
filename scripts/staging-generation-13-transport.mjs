/** Disabled generation-13 orchestration over the reviewed shared transports. */
import { createHash } from 'node:crypto'
import { buildGeneration13CredentialSql, createGeneration13DispatchJournal, GENERATION, IDENTITIES, MAX_WINDOW_MS, PACKAGE_ID, PROJECT_REF, WINDOW_ID } from './staging-generation-13-credentials.mjs'
...
export const NATIVE_GENERATION_13_TRANSPORT_ENABLED=false
...
const unavailable=()=>{throw new Error('Generation-13 transport unavailable')}
...
function validateReceipt(rows,expiresAt){
  if(!Array.isArray(rows)||rows.length!==1||Object.keys(rows[0]??{}).join('|')!=='tll_generation_13_credential_receipt')unavailable()
  const value=rows[0].tll_generation_13_credential_receipt,expected={controlsEnabled:false,expiresAt,generation:GENERATION,packageId:PACKAGE_ID,projectRef:PROJECT_REF,runtimeCount:5,status:'PASS',windowId:WINDOW_ID}
...
export async function executeGeneration13CredentialWindow({ports,journal=createGeneration13DispatchJournal(),now=Date.now,randomBytes,randomUUID,onPhase}={}){
...
export const GENERATION_13_SOURCE_FINGERPRINT=createHash('sha256').update(`${PACKAGE_ID}|${WINDOW_ID}|${GENERATION}`).digest('hex')
