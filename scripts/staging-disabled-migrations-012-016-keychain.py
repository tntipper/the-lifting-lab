"""Disabled reader for exactly the existing Supabase CLI Keychain item."""
import base64
import ctypes
import os
import re
...
def unavailable():
    raise RuntimeError("Disabled staging migration install unavailable")
...
def read_exact():
    ...
    security.SecItemCopyMatching.argtypes = [ptr, ctypes.POINTER(ptr)]; security.SecItemCopyMatching.restype = ctypes.c_int32
    ...
    try:
        if not service or not account: unavailable()
        ...
        if not query or security.SecItemCopyMatching(query, ctypes.byref(result)) != 0 or not result.value: unavailable()
        length = cf.CFDataGetLength(result)
        if length < 1 or length > 256: unavailable()
