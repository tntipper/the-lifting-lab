/**
 * Secret-free Gen 21 pre-arm / Phase 2 gate.
 *
 * Refuses arming Gen 21 unless:
 * 1. Gen 19 + Gen 21 recovery successor pins match their credentials packages, AND
...
import { GENERATION, PACKAGE_ID, PREDECESSOR, WINDOW_ID } from './staging-generation-21-credentials.mjs'
import { NATIVE_GENERATION_21_TRANSPORT_ENABLED } from './staging-generation-21-transport.mjs'
import { NATIVE_GENERATION_21_DATABASE_TRANSPORT_ENABLED } from './staging-generation-21-database-transport.mjs'
...
const unavailable = (reason) => {
  const error = new Error(`Generation-21 pre-arm unavailable: ${reason}`)
...
/** Validate complete secret-free retired-role baseline evidence (no SQL, secrets, or tokens). */
export function projectPredecessorRetirementEvidence(value) {
  ...
  if (value.status !== 'PASS_RETIRED_BASELINE') return null
  if (value.projectRef !== 'qdmvngjwkcsilzmqksme') return null
  ...
  // `passwordsConfigured` is an aggregate zero count, not credential material.
  if (/SCRAM|secret|token|BEGIN;|sbp_/i.test(JSON.stringify(value))) return null
  return Object.freeze({ ...value })
}

/**
 * Confirm the reviewed Gen19 retirement plan and Gen21 checklist carry the
 ...
 * When `requireArmedGatesFalse` is true (default), refuses if Gen 21 native gates are already true
 * without a separate arming review — Phase 1 must stay disarmed.
 */
export async function assertGeneration21PreArmReady({
  retirementEvidence = null,
  requireArmedGatesFalse = true,
} = {}) {
  if (GENERATION_21_CONSUMED) unavailable('generation21_consumed_no_replay')
  if (GENERATION !== 21 || WINDOW_ID !== 'a5511645-77af-4fc9-9e4c-f5c8a474d5fa' || PACKAGE_ID !== 'tll-staging-generation-21-credentials/v1') {
  ...
  await assertRecoverySuccessorPinsMatchCredentials([17, 18, 19, 20, 21])

  if (requireArmedGatesFalse) {
    if (NATIVE_GENERATION_21_TRANSPORT_ENABLED !== false || NATIVE_GENERATION_21_DATABASE_TRANSPORT_ENABLED !== false) {
  ...
  const nativeGatesArmed = NATIVE_GENERATION_21_TRANSPORT_ENABLED === true
  ...
  return Object.freeze({
    ...
    predecessorBaselineComplete: Boolean(proven),
...
export const DEFAULT_LOCAL_RETIREMENT_EVIDENCE_PATH =
  'implementation-state/staging/tll-generation-21-predecessor-retirement-evidence.json'
export const RETIREMENT_EVIDENCE_PATH_ENV = 'TLL_GENERATION_21_RETIREMENT_EVIDENCE_PATH'
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    const result = await assertGeneration21PreArmReady({
      retirementEvidence,
