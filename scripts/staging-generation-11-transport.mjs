/** Disabled generation-11 orchestration over the reviewed shared transports. */
import { createHash } from 'node:crypto'
import { buildGeneration11CredentialSql, createGeneration11DispatchJournal, GENERATION, IDENTITIES, MAX_WINDOW_MS, PACKAGE_ID, PROJECT_REF, WINDOW_ID } from './staging-generation-11-credentials.mjs'
...
export const NATIVE_GENERATION_11_TRANSPORT_ENABLED=false
...
const unavailable=()=>{throw new Error('Generation-11 transport unavailable')}
...
function validateReceipt(rows,expiresAt){
  if(!Array.isArray(rows)||rows.length!==1||Object.keys(rows[0]??{}).join('|')!=='tll_generation_11_credential_receipt')unavailable()
  const value=rows[0].tll_generation_11_credential_receipt,expected={controlsEnabled:false,expiresAt,generation:GENERATION,packageId:PACKAGE_ID,projectRef:PROJECT_REF,runtimeCount:5,status:'PASS',windowId:WINDOW_ID}
...
export async function executeGeneration11CredentialWindow({ports,journal=createGeneration11DispatchJournal(),now=Date.now,randomBytes,randomUUID,onPhase}={}){
  ...
  try{
    ...
    await notify('DATABASE_PACKAGE');const verifiers=Object.fromEntries(purposes.map((purpose,index)=>[purpose,deriveScramVerifier(projection.passwords[purpose],Buffer.alloc(18,index+1))]));const sql=buildGeneration11CredentialSql({expiresAt,verifiers,nowMs})
...
export const GENERATION_11_SOURCE_FINGERPRINT=createHash('sha256').update(`${PACKAGE_ID}|${WINDOW_ID}|${GENERATION}`).digest('hex')
