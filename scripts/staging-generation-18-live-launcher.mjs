#!/usr/bin/env node
/**
 * Dedicated Generation 18 live launcher.
 *
 ...
 *     `scripts/staging-generation-18-run-live-once.mjs` for the whole run
 ...
 * 1. Start via `scripts/staging-generation-18-run-live-once.mjs` in a long-lived process
...
import {
  ...
  PROJECT_REF,
  WINDOW_ID,
} from './staging-generation-18-credentials.mjs'
import {
  executeGeneration18CredentialWindow,
  NATIVE_GENERATION_18_TRANSPORT_ENABLED,
} from './staging-generation-18-transport.mjs'
import { NATIVE_GENERATION_18_DATABASE_TRANSPORT_ENABLED } from './staging-generation-18-database-transport.mjs'
import { createStagingWindowPhaseJournal } from './staging-window-phase-journal.mjs'
import { assertLongSessionContract } from './staging-generation-18-long-session-contract.mjs'
import {
  persistConnectionFailureEvidence,
  secretFreeLauncherTerminal,
} from './staging-generation-18-connection-failure-evidence.mjs'
...
export {
  ...
  KEEPALIVE_SCHEMA,
  LONG_SESSION_ENV,
} from './staging-generation-18-long-session-contract.mjs'
...
function gatesArmed() {
  return NATIVE_GENERATION_18_TRANSPORT_ENABLED === true
    && NATIVE_GENERATION_18_DATABASE_TRANSPORT_ENABLED === true
}

/** Fail-closed native entry. Returns disabled status unless every gate is armed. */
export async function runNativeGeneration18CredentialWindow({
  ...
  try {
    ...
    const [
      { stageVercelSecrets, stageSupabaseSecrets, readbackProviderNames, removeVercelSecrets, removeSupabaseSecrets },
      {
        ...
        dispatchGeneration18Database,
        recoverGeneration18Database,
        verifyGeneration18EntryBaseline,
      ...
      import('./staging-generation-18-database-transport.mjs'),
    ...
    const result = await executeGeneration18CredentialWindow({
      onPhase,
      ports: {
        preflightDatabase: () => verifyGeneration18EntryBaseline({ token }),
        stageVercel: stageVercelSecrets,
        stageSupabase: stageSupabaseSecrets,
        readbackNames: readbackProviderNames,
        dispatchDatabase: sql => dispatchGeneration18Database(sql, { token }),
        verifyConnections: async input => {
          try {
            await verifyGeneration6Connections({ ...input, tlsCa, createRuntime: createStagingPostgresRuntime })
          } catch (error) {
            const projected = Error('Generation-18 connection verification unavailable')
          ...
          try {
            ...
            await verifyGeneration18ZeroSessionsAfterPoolerDrain({ token })
          } catch (error) {
        ...
        recoverDatabase: () => recoverGeneration18Database({ token }),
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    const result = await runNativeGeneration18CredentialWindow()
