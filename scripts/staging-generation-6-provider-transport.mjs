...
export function runPrivateCli(args, input, inputFd = 0) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(NPX, args, { stdio, env: safeEnvironment() }); const chunks=[]; let size=0, done=false
...
export async function stageSupabaseSecrets(secrets,{run=runPrivateCli}={}) {
  validateMap(secrets,GENERATED_SUPABASE_SECRET_NAMES);const input=encodeDotenv(secrets)
  try { await run(['--yes','supabase','secrets','set','--env-file','/dev/fd/3','--project-ref',PROJECT_REF,'--output','json'],input,3) }
...
export async function readbackProviderNames({run=runPrivateCli}={}) {
  const empty=Buffer.alloc(0)
  const [supabaseRaw,vercelRaw]=await Promise.all([
    run(['--yes','supabase','secrets','list','--project-ref',PROJECT_REF,'--output','json'],empty,0),
