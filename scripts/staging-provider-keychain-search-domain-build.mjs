#!/usr/bin/env node
/** Source-bound build/check for the read-only search-domain diagnostic. Never runs it. */
...
const source = resolve(import.meta.dirname, 'staging-provider-keychain-search-domain-diagnostic.swift')
const privateRoot = resolve(root, '../implementation-state')
const directory = resolve(privateRoot, 'staging')
const name = 'tll-provider-keychain-search-domain-armed-v1'
const binary = resolve(directory, name)
const receipt = `${binary}.build.json`
const fail = () => { throw Error('Search-domain build unavailable') }
...
export function armedSearchDomainSource(body) {
  const securityCalls = typeof body === 'string'
    ? [...body.matchAll(/\b(Sec[A-Za-z0-9_]+)\s*\(/g)].map(match => match[1]) : []
  const allowed = new Set(['SecKeychainGetPreferenceDomain', 'SecKeychainCopySearchList',
    'SecKeychainCopyDomainSearchList', 'SecKeychainGetPath', 'SecKeychainGetTypeID'])
  return typeof body === 'string'
    && (body.match(/\btllSearchDomainDiagnosticEnabled\s*=\s*(?:true|false)\b/g) ?? []).length === 1
    && /\btllSearchDomainDiagnosticEnabled\s*=\s*true\b/.test(body)
...
export function validSearchDomainArtifact(record, actual, binaryStat, receiptStat, uid) {
  return record && typeof record === 'object' && !Array.isArray(record)
    ...
    && record.schema === 'tll-search-domain-armed-build/v1'
...
function check() {
  ...
  if (!armedSearchDomainSource(fs.readFileSync(source, 'utf8'))) fail()
  ...
  try {
    ...
    scratch = resolve(directory, `.tll-search-domain-check-${randomUUID()}`)
    ...
    if (!validSearchDomainArtifact(record, actual, binaryArtifact.stat, receiptArtifact.stat,
      process.getuid())) fail()
    return Object.freeze({ status: 'ARMED_SEARCH_DOMAIN_BINARY_VERIFIED', ...record })
  } finally {
...
function build() {
  ...
  if (!armedSearchDomainSource(bytes.toString('utf8')) || fs.existsSync(binary)
    || fs.existsSync(receipt)) fail()
  const scratch = resolve(directory, `.tll-search-domain-build-${randomUUID()}`)
  fs.mkdirSync(scratch, { mode: 0o700 })
  ...
  try {
    ...
    const record = Object.freeze({ schema: 'tll-search-domain-armed-build/v1',
