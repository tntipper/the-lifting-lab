"""Disabled exact-selector Keychain reader for a future one-run staging rotation."""
import os
import subprocess
import sys

APPROVED_BROKER_ROTATION = False
_SELECTORS = {
...
def unavailable():
    raise RuntimeError("Staging broker rotation credential unavailable")


def main():
    if not APPROVED_BROKER_ROTATION or sys.platform != "darwin" or len(sys.argv) != 2:
        unavailable()
    selector = sys.argv[1]
