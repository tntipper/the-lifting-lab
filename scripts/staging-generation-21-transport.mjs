/** Reviewed one-window Generation 21 orchestration over the shared transports. */
import { createHash } from 'node:crypto'
import { ACTIVE_WINDOW_EXPIRES_AT, assertGeneration21ActiveWindowExpiry, buildGeneration21CredentialSql, createGeneration21DispatchJournal, GENERATION, IDENTITIES, PACKAGE_ID, PROJECT_REF, WINDOW_ID } from './staging-generation-21-credentials.mjs'
...
export const NATIVE_GENERATION_21_TRANSPORT_ENABLED=false
...
const unavailable=()=>{throw new Error('Generation-21 transport unavailable')}
...
function validateReceipt(rows,expiresAt){
  if(!Array.isArray(rows)||rows.length!==1||Object.keys(rows[0]??{}).join('|')!=='tll_generation_21_credential_receipt')unavailable()
  const value=rows[0].tll_generation_21_credential_receipt,expected={controlsEnabled:false,expiresAt,generation:GENERATION,packageId:PACKAGE_ID,projectRef:PROJECT_REF,runtimeCount:5,status:'PASS',windowId:WINDOW_ID}
...
export async function executeGeneration21CredentialWindow({ports,journal=createGeneration21DispatchJournal(),now=Date.now,randomBytes,randomUUID,onPhase}={}){
  ...
  const nowMs=now()
  let expiresAt
  try{expiresAt=assertGeneration21ActiveWindowExpiry(nowMs)}catch{
    return Object.freeze({status:'WINDOW_EXPIRY_UNSET',target:PROJECT_REF,generation:GENERATION,windowId:WINDOW_ID,
  ...
  try{
    ...
    await notify('DATABASE_PACKAGE');const verifiers=Object.fromEntries(purposes.map((purpose,index)=>[purpose,deriveScramVerifier(projection.passwords[purpose],Buffer.alloc(18,index+1))]));const sql=buildGeneration21CredentialSql({verifiers,nowMs})
    ...
    const managementStatusCode=Number.isInteger(error?.managementStatusCode)&&error.managementStatusCode>=100&&error.managementStatusCode<=599
      ?error.managementStatusCode:undefined
    ...
    return Object.freeze({
      ...
      ...(managementStatusCode!==undefined?{managementStatusCode}:{}),
...
export const GENERATION_21_SOURCE_FINGERPRINT=createHash('sha256').update(`${PACKAGE_ID}|${WINDOW_ID}|${GENERATION}`).digest('hex')
