...
import { PREDECESSOR, PROJECT_REF, PACKAGE_ID, WINDOW_ID } from './staging-generation-15-credentials.mjs'

export const NATIVE_GENERATION_15_DATABASE_TRANSPORT_ENABLED = false
...
const unavailable=()=>{throw new Error('Generation-15 database transport unavailable')}
...
export function readSupabaseTokenFromKeychain(){
  if(!NATIVE_GENERATION_15_DATABASE_TRANSPORT_ENABLED||process.platform!=='darwin')unavailable()
  const helper=fileURLToPath(new URL('./staging-generation-15-keychain.py',import.meta.url))
...
export async function postManagementQuery(token,query,{request=https.request}={}){
  ...
  try{return await new Promise((resolve,reject)=>{let req,done=false,size=0;const chunks=[]
    ...
    const fail=()=>{try{req?.destroy()}catch{};finish(new Error('Generation-15 database transport unavailable'))};const timer=setTimeout(fail,TIMEOUT_MS)
...
export async function dispatchGeneration15Database(sql,{token,post=postManagementQuery}={}){
  if(typeof sql!=='string'||!sql.startsWith('BEGIN;\n')||!sql.endsWith(' AS tll_generation_15_credential_receipt;\n')
    ||sql.split('tll_generation_15_credential_receipt').length!==2||!sql.includes(PACKAGE_ID)||!sql.includes(WINDOW_ID))unavailable()
...
export const GENERATION_15_ENTRY_BASELINE_SQL=`BEGIN READ ONLY;
...
DO $preflight$ DECLARE r text; role_marker text; parsed_marker jsonb; BEGIN
 IF current_database()<>'postgres' OR current_user<>'postgres' OR session_user<>'postgres'
  ...
  RAISE EXCEPTION 'Generation 15 entry operator mismatch'; END IF;
 IF NOT EXISTS(SELECT FROM tll_staging_private.environment WHERE singleton AND environment='tll-hosted-staging-v1'
  AND operator_project_ref='${PROJECT_REF}') THEN RAISE EXCEPTION 'Generation 15 entry environment mismatch'; END IF;
 IF (SELECT count(*) FROM pg_roles WHERE rolname IN ('tll_customer_runtime','tll_cart_runtime','tll_broker_runtime','tll_provisional_runtime','tll_bridge_runtime'))<>5
  ...
  RAISE EXCEPTION 'Generation 15 entry predecessor mismatch'; END IF;
 FOREACH r IN ARRAY ARRAY['tll_customer_runtime','tll_cart_runtime','tll_broker_runtime','tll_provisional_runtime','tll_bridge_runtime'] LOOP
  ...
  IF role_marker IS NULL OR role_marker !~ '^tll-runtime-window/v1 [{].*[}]$' THEN RAISE EXCEPTION 'Generation 15 entry predecessor marker malformed: %',r; END IF;
  BEGIN parsed_marker:=substring(role_marker FROM '^tll-runtime-window/v1 ([{].*[}])$')::jsonb;
  EXCEPTION WHEN OTHERS THEN RAISE EXCEPTION 'Generation 15 entry predecessor marker invalid: %',r; END;
  IF parsed_marker IS DISTINCT FROM '${predecessorMarkerPayload}'::jsonb THEN RAISE EXCEPTION 'Generation 15 entry predecessor marker mismatch: %',r; END IF;
 END LOOP;
 IF EXISTS(SELECT FROM (VALUES ((SELECT enabled FROM tll_customer_private.control WHERE singleton)),((SELECT enabled FROM tll_cart_private.control WHERE singleton)),((SELECT enabled FROM tll_broker_private.control WHERE singleton)),((SELECT enabled FROM tll_provisional_private.control WHERE singleton)),((SELECT enabled FROM tll_bridge_private.control WHERE singleton))) controls(enabled) WHERE enabled) THEN
  RAISE EXCEPTION 'Generation 15 entry control enabled'; END IF;
END $preflight$;
COMMIT;
SELECT jsonb_build_object('status','ENTRY_BASELINE_PASS','projectRef','${PROJECT_REF}','windowId','${WINDOW_ID}','runtimeGeneration',14,'runtimeInert',true,'controlsEnabled',false) AS tll_generation_15_entry_baseline;
`
export async function verifyGeneration15EntryBaseline({token,post=postManagementQuery}={}){
  const rows=await post(token,GENERATION_15_ENTRY_BASELINE_SQL)
  if(!Array.isArray(rows)||rows.length!==1||Object.keys(rows[0]??{}).join('|')!=='tll_generation_15_entry_baseline')unavailable()
  const value=rows[0].tll_generation_15_entry_baseline,expected={controlsEnabled:false,projectRef:PROJECT_REF,runtimeGeneration:14,runtimeInert:true,status:'ENTRY_BASELINE_PASS',windowId:WINDOW_ID}
...
SET LOCAL statement_timeout='15s';
DO $verify$ BEGIN
 IF EXISTS(SELECT FROM pg_stat_activity WHERE backend_type='client backend' AND usename IN ('tll_customer_runtime','tll_cart_runtime','tll_broker_runtime','tll_provisional_runtime','tll_bridge_runtime')) THEN RAISE EXCEPTION 'Generation 15 runtime sessions remain'; END IF;
 IF EXISTS(SELECT FROM (VALUES ((SELECT enabled FROM tll_customer_private.control WHERE singleton)),((SELECT enabled FROM tll_cart_private.control WHERE singleton)),((SELECT enabled FROM tll_broker_private.control WHERE singleton)),((SELECT enabled FROM tll_provisional_private.control WHERE singleton)),((SELECT enabled FROM tll_bridge_private.control WHERE singleton))) controls(enabled) WHERE enabled) THEN RAISE EXCEPTION 'Generation 15 control enabled during zero-session proof'; END IF;
END $verify$;
COMMIT;
SELECT jsonb_build_object('status','ZERO_SESSIONS','projectRef','${PROJECT_REF}','windowId','${WINDOW_ID}','controlsEnabled',false) AS tll_generation_15_zero_sessions;
`
export async function verifyGeneration15ZeroSessions({token,post=postManagementQuery}={}){
  const rows=await post(token,zeroSessionSql)
  if(!Array.isArray(rows)||rows.length!==1||Object.keys(rows[0]??{}).join('|')!=='tll_generation_15_zero_sessions')unavailable()
  const value=rows[0].tll_generation_15_zero_sessions,expected={controlsEnabled:false,projectRef:PROJECT_REF,status:'ZERO_SESSIONS',windowId:WINDOW_ID}
  if(!value||Object.keys(value).sort().join('|')!==Object.keys(expected).sort().join('|'))unavailable()
  for(const [key,wanted]of Object.entries(expected))if(value[key]!==wanted)unavailable()
  return Object.freeze(expected)
}

const recoverySql=()=>readFileSync(new URL('../config/staging-generation-15-recovery.sql',import.meta.url),'utf8')+
  `\nSELECT jsonb_build_object('status','RECOVERY_COMMITTED','projectRef','${PROJECT_REF}','windowId','${WINDOW_ID}') AS tll_generation_15_recovery_receipt;\n`
const postCommitSql=()=>readFileSync(new URL('../config/staging-generation-15-recovery-postcommit.sql',import.meta.url),'utf8')+
  `\nSELECT jsonb_build_object('status','RECOVERY_VERIFIED','projectRef','${PROJECT_REF}','windowId','${WINDOW_ID}') AS tll_generation_15_recovery_postcommit;\n`
...
export async function recoverGeneration15Database({token,post=postManagementQuery}={}){
  exact(await post(token,recoverySql()),'tll_generation_15_recovery_receipt','RECOVERY_COMMITTED')
  exact(await post(token,postCommitSql()),'tll_generation_15_recovery_postcommit','RECOVERY_VERIFIED')
